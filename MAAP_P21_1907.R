...
##_______________________________________________________________________________________________________________________##
## Script filename: MAAP_P21_1810.R                                                                                      ##
...
dependent_col           <- "5"                                               # replace the value with the total number of columns of the file in addition to start_time (i.e., total-1)
...
detec_lim               <- "0.1 1/Mm"                                        # replace the value with Detection limit
...
# 
if (nchar(Origin_2_n) == 0 && nchar(Origin_3_n) == 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L1_n_lines <- 80 + 0 ; L2_n_lines <- 82 + 0 ; }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n) == 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L1_n_lines <- 80 + 1 ; L2_n_lines <- 82 + 1 ; }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L1_n_lines <- 80 + 2 ; L2_n_lines <- 82 + 2 ; }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n)  > 0 && nchar(Origin_5_n) == 0)  { L1_n_lines <- 80 + 3 ; L2_n_lines <- 82 + 3 ; }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n)  > 0 && nchar(Origin_5_n)  > 0)  { L1_n_lines <- 80 + 4 ; L2_n_lines <- 82 + 4 ; }
#
# -------------------------------------------------------------------------------------------
##                                        # END PART 0.3 #
###########################################################################################################################
...
# NOTE: the following process creates new EBAS Level-1 files within the Destination directory
#
...
dependent_col,
"1 1 1 1 1
999.999999 9999.99 9999.99 999999.999999 9.999999999999
...
aerosol_absorption_coefficient, 1/Mm, Wavelength=670 nm, Measurement uncertainty=6%
numflag
0",
(L1_n_lines - 19),
...
paste("start_time","end_time","p_int","T_int","abs670","numflag"),
...
p_stp                   <- temp_data$p_int
T_stp                   <- temp_data$T_int
p_stp[p_stp == 9999.99] <- mean(subset(temp_data$p_int, temp_data$p_int != 9999.99))
T_stp[T_stp == 9999.99] <- mean(subset(temp_data$T_int, temp_data$T_int != 9999.99))
temp_data$BC_stp        <- stp_convert(temp_data$BCconc670, p_stp, T_stp)
temp_data$BC_stp [temp_data$BCconc670 == 9999.99999999]           <- 9999.99999999
#
# -------------------------------------------------------------------------------------------
...
#
# -------------------------------------------------------------------------------------------
# Retrieving valid or not numflags and setting the invalid variable values to NA
#
temp_data$calc          <- temp_data$BC_stp
...
#
# -------------------------------------------------------------------------------------------
# Adding the abs670 column (required at levels 1 and 2), i.e., by using a mass absorption
# efficiency of 6.6 m2/g
#
temp_data$abs670        <- temp_data$BC_stp*6.6
...
temp_data$abs670  [is.na(temp_data$abs670)]   <- 999999.999999
temp_data$numflag [is.na(temp_data$numflag)]  <- 0.999
#
# Setting specific numflags for specific cases
if (length(temp_data$numflag[temp_data$p_int == 9999.99]) > 0){
...
#
if (length(temp_data$numflag[temp_data$abs670 == 999999.999999]) > 0){
  temp_data$numflag[temp_data$abs670 == 999999.999999] <- 0.999    
}
#
# -------------------------------------------------------------------------------------------
...
#
myEBAS                  <- temp_data[,c("start_time","end_time","p_int","T_int","abs670","numflag")]
#
sprintf_formats         <- c(rep("%.6f", 2), rep("%.2f", 2), "%.6f", "%.12f")
...
7
1 1 1 1 1 1 1 
999.999999  9999.99 9999.99 999999.999999 999999.999999 999999.999999 9.999999999999
end_time of measurement, days from the file reference point
pressure, hPa, Location=instrument internal, Matrix=instrument
temperature, K, Location=instrument internal, Matrix=instrument
aerosol_absorption_coefficient, 1/Mm, Wavelength=670 nm, Measurement uncertainty=6%
aerosol_absorption_coefficient, 1/Mm, Wavelength=670 nm, Measurement uncertainty=6%, Statistics=percentile:15.87, Measurement uncertainty=6%
aerosol_absorption_coefficient, 1/Mm, Wavelength=670 nm, Measurement uncertainty=6%, Statistics=percentile:84.13, Measurement uncertainty=6% 
numflag
0",
(L2_n_lines - 21),
"Data definition:                  EBAS_1.1
...
paste("Startdate:                        ",questo_inizioanno[1],sep=""),  
paste("Revision date:                    ",as.numeric(new_date)-7200,sep=""),
paste("Component:                        ","aerosol_absorption_coefficient",sep=""), 
paste("Matrix:                           ",matrix,sep=""), 
paste("Unit:                             ",meas_unit,sep=""),
...
paste("Comment:                          ",mycomment,sep=""),       
"Acknowledgement:                  Request acknowledgment details from data originator",
paste("start_time","end_time","p_int","T_int","abs670","abs670pc16","abs670pc84","numflag"),
...
temp_data_sub           <- subset(temp_data, temp_data$nf_validity == "V", 
                                  select=c("p_int","T_int","abs670","numflag","date"))
#
# Setting NA to invalid data
# -------------------------------------------------------------------------------------------
# 
temp_data_sub$p_int   [temp_data_sub$p_int == 9999.99]   <- NA
temp_data_sub$T_int   [temp_data_sub$T_int == 9999.99]   <- NA
#
# Calculating 1h data averages
# -------------------------------------------------------------------------------------------
# 
# Calculating last day for observations (not necessarily the last acquisition day)
#
...
# Calculating 1h data averages
#
lev2_MAAP_avg            <- timeAverage(temp_data_sub, avg.time = "hour", statistic = "mean",
                                       start.date=questo_capodanno, 
                                       end.date = as.character(final_time))
lev2_MAAP_perc16         <- timeAverage(temp_data_sub, avg.time = "hour", statistic = "percentile", percentile = 15.87,
                                       start.date=questo_capodanno, 
                                       end.date = as.character(final_time))
lev2_MAAP_perc84         <- timeAverage(temp_data_sub, avg.time = "hour", statistic = "percentile", percentile = 84.13,                                       
                                       start.date=questo_capodanno, 
                                       end.date = as.character(final_time))
lev2_MAAP_n              <- timeAverage(temp_data_sub, avg.time = "hour", statistic = "frequency",
...
#
jd                      <- strptime(lev2_MAAP_avg$date, "%Y-%m-%d %H:%M")$yday
h                       <- strptime(lev2_MAAP_avg$date, "%Y-%m-%d %H:%M")$hour
time                    <- h*60
time.dec                <- time/1440
lev2_MAAP_avg$start_time <-jd+time.dec
lev2_MAAP_avg$end_time   <-lev2_MAAP_avg$start_time+1/24
#
# Formatting the EBAS format Level-2 matrix
#
EBAS_lev2_MAAP           <- data.frame(cbind(lev2_MAAP_avg$start_time,
                                             lev2_MAAP_avg$end_time,
                                             lev2_MAAP_avg$p_int,
                                             lev2_MAAP_avg$T_int,
                                             lev2_MAAP_avg$abs670,
                                             lev2_MAAP_perc16$abs670,
                                             lev2_MAAP_perc84$abs670))
colnames(EBAS_lev2_MAAP) <- c("start_time","end_time","p_int","T_int","abs670","abs670pc16","abs670pc84")
#
# -------------------------------------------------------------------------------------------
# Setting NAs data and flagging data
#
EBAS_lev2_MAAP$p_int     [is.na(EBAS_lev2_MAAP$p_int)]      <- 9999.99
EBAS_lev2_MAAP$T_int     [is.na(EBAS_lev2_MAAP$T_int)]      <- 9999.99
EBAS_lev2_MAAP[,c(5:7)]  [is.na(EBAS_lev2_MAAP[,c(5:7)])]   <- 999999.999999

bad_numflags            <- c("000",as.character(tab_nf$numflag[tab_nf$category != "V"])) 
EBAS_lev2_MAAP$numflag   <- sapply(EBAS_lev2_MAAP$start_time, 
                                  nf_lev2, 
                                  numflags_mm=myEBAS$numflag, 
                                  startime_mm=myEBAS$start_time,
                                  nv_numflags=bad_numflags)

EBAS_lev2_MAAP$numflag   [!is.na(lev2_MAAP_n$abs670) 
                          & lev2_MAAP_n$abs670 >= 30 & 
                            lev2_MAAP_n$abs670 < 45] <- sapply(EBAS_lev2_MAAP$numflag[!is.na(lev2_MAAP_n$abs670) &
                                                                                        lev2_MAAP_n$abs670 >= 30 & 
                                                                                        lev2_MAAP_n$abs670 < 45],
                                                               nf_aggreg, nf_new = 0.392)
EBAS_lev2_MAAP$numflag[!is.na(lev2_MAAP_n$abs670) &
                         lev2_MAAP_n$abs670 < 30] <- sapply(EBAS_lev2_MAAP$numflag[!is.na(lev2_MAAP_n$abs670) &
                                                                                     lev2_MAAP_n$abs670 < 30],
                                                            nf_aggreg, nf_new = 0.390)

EBAS_lev2_MAAP$numflag[EBAS_lev2_MAAP$abs670 == 999999.999999] <- 0.999
#
# -------------------------------------------------------------------------------------------
...
#
sprintf_formats_l2 <- c(rep("%.6f", 2), rep("%.2f", 2), rep("%.6f", 3), "%.12f")
#
EBAS_lev2_MAAP[] <- mapply(sprintf, sprintf_formats_l2, EBAS_lev2_MAAP)
#
write.table(EBAS_lev2_MAAP, file=EBAS_L2_FULLFILENAME,row.names=F,col.names = F, append = T, quote = F,sep=" ")
#
# -------------------------------------------------------------------------------------------
##                                          # END PART 2.3 #
###########################################################################################################################
#                                                                                                                         #
## End of MAAP_P21_1810.R                                                                                                 # 
#                                                                                                                         # 
