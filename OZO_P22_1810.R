...
##_______________________________________________________________________________________________________________________##
## Script filename: OZO_D22_1810.R                                                                                       ##
...
#
inst_type               <- "OZO"                                             # replace the value with your instrument type
#
##                                        # END PART 0.2 #
###########################################################################################################################
...
OZO_L01_today_Y          <- subset(temp_L01, nf_validity == "V")
# -------------------------------------------------------------------------------------------
# Reading Level-2
...
temp_L02$numflag        <-temp_L02$flag_o3

OZO_L02_today           <-data.frame(temp_L02$date,temp_L02$o3,temp_L02$numflag)
#
colnames(OZO_L02_today) <- c("date","o3","numflag")
#
OZO_L02_today$month     <- format(OZO_L02_today$date,"%B")
OZO_L02_today$monthNum  <- as.numeric(format(OZO_L02_today$date,"%m"))
OZO_L02_today$day       <- as.numeric(format(OZO_L02_today$date,"%d"))
#
OZO_FAIL_today          <- subset(OZO_L02_today, numflag == 0.999)
#
TFAIL                   <- subset(OZO_FAIL_today, select=c("month","day","monthNum","numflag"))
...
if (nrow(mesi_FAIL) == 0)  { }
#
OZO_L02_today_Y        <- subset(OZO_L02_today, numflag != 0.999)

unico                   <- c(OZO_L02_today_Y[!duplicated(OZO_L02_today_Y[,c('month')]),]$month)
...
#
unico_M               <- c(OZO_L02_today_Y[!duplicated(OZO_L02_today_Y[,c('monthNum')]),]$monthNum)

for (qm in unico_M)
{
  OZO_L02_ThisMonth   <-subset(OZO_L02_today_Y, as.integer(monthNum)==qm)
  ...
  #
  FAIL_ThisMonth      <- subset(OZO_FAIL_today, as.integer(monthNum) == qm)
  ...
  mensile             <-subset(OZO_L02_ThisMonth, numflag != 0.999)
  ...
  #
  mensile$mean        <-mean(mensile$o3)
  mensile$sd          <-sd(mensile$o3)
  men_MIN             <-subset(mensile, o3 == min(mensile$o3))
  men_MAX             <-subset(mensile, o3 == max(mensile$o3))
  # -------------------------------------------------------------------------------------------
  # Cleaning Destination directory
  # 
  ...
  #
  plot(ylim=c(-0.1,100),
       xlim = c(min(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
        mgp = c(8, 4, 0),
       mensile$date, mensile$o3, type = "h",
       xlab = "",ylab =bquote(paste("O"[3], " (nmol/mol) - L02")), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  title(bquote(bold(paste("O"[3], " - L02 - ",.(questo_anno)," ",.(This_Month_Name)))), line = -3)
  lines(OZO_L02_ThisMonth$date, mensile$o3, type = "l", lty = 1, col="darkred",lwd = 2)
  
  points(men_MAX$date[1], men_MAX$o3[1], col= "magenta",pch=20)
  text(men_MAX$date[1], -0.1, labels = paste("Max (", format(men_MAX$date[1],"%B %d"), ")", sep=""),col="magenta",pos=3,cex = 1.8)
  segments(men_MAX$date[1], men_MAX$o3[1], men_MAX$date[1],-1, lty = 2, col="black",lwd = 1)

  points(men_MIN$date[1], men_MIN$o3[1], col= "blue",pch=20)
  text(men_MIN$date[1], -0.1, labels = paste("Min (", format(men_MIN$date[1],"%B %d"), ")", sep =""),col="blue",pos=3,cex = 1.8)
  segments(men_MIN$date[1], men_MIN$o3[1], men_MIN$date[1], -1, lty = 2, col="black",lwd = 1)
  ...
  #
  temp_L01_ThisMonth$Flow_A[temp_L01_ThisMonth$Flow_A > 90] <- 0
  temp_L01_ThisMonth$Flow_B[temp_L01_ThisMonth$Flow_B > 90] <- 0
  
  plot(ylim=c(mean(temp_L01_ThisMonth$Flow_A)-0.5,mean(temp_L01_ThisMonth$Flow_A)+0.5),
       xlim = c(min(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
       temp_L01_ThisMonth$date, temp_L01_ThisMonth$Flow_A, type = "l", lwd = 0.5,
       xlab = "",ylab =("Flow_A & Flow_B - L01"), col="thistle1", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))

  title(paste("Flow_A & Flow_B - L01 -",questo_anno,This_Month_Name), line = -3)
  
  lines(temp_L01_ThisMonth$date, temp_L01_ThisMonth$Flow_A, type = "l", lty = 1, col="darkred",lwd = 1.5)
  lines(temp_L01_ThisMonth$date, temp_L01_ThisMonth$Flow_B, type = "l", lty = 1, col="darkblue",lwd = 0.5)
  
  segments(head(temp_L01_ThisMonth$date), 0.35, tail(temp_L01_ThisMonth$date), 0.35, lty = 6, col="red",lwd = 0.5)
  text(temp_L01_ThisMonth$date[1], 0.32, labels = c("Warning threshold (Flow 0.35)"),col="red",pos=4,cex = 1.8)
  
  legend("topleft", legend=c("Flow_A","Flow_B"), lty=c(1,1), lwd = c(2,2), col=c("darkred","darkblue"), cex = 2.5)
  #
  # -------------------------------------------------------------------------------------------
  # -------------------------------------------------------------------------------------------
  # Plotting SPAN
  ...
  plot(ylim=c(-50,150),
       xlim = c(min(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
       mgp = c(8, 4, 0),
       temp_L00_ThisMonth$date, temp_L00_ThisMonth$o3, type = "n",
       xlab = "",ylab =("Span & Zero O3 - L00"), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  lines(temp_L00_ThisMonth$date, temp_L00_ThisMonth$p_int, type = "l", lty = 1, col="darkblue",lwd = 0.5)
  title(paste("Span and Zero O3 - L00 -",questo_anno,This_Month_Name), line = -3)
  
  SPAN_span<-subset(temp_L00_ThisMonth,(temp_L00_ThisMonth$numflag==0.682 & temp_L00_ThisMonth$o3 > 70))
  SPAN_zero<-subset(temp_L00_ThisMonth,(temp_L00_ThisMonth$numflag==0.682 & temp_L00_ThisMonth$o3 < 10))
  
  xx_S <- c(SPAN_span$date, rev(SPAN_span$date))
  yy_S <- c(rep(0, nrow(SPAN_span)), rev(SPAN_span$o3))
  xx_Z <- c(SPAN_zero$date, rev(SPAN_zero$date))
  yy_Z <- c(rep(0, nrow(SPAN_zero)), rev(SPAN_zero$o3))
  
  points(xx_S, yy_S, col='darkred' ,pch = 25, cex = 2.5)
  points(xx_Z, yy_Z, col='darkblue',pch = 24, cex = 2.5)
  
  text(temp_L00_ThisMonth$date[1], 67, labels = c(paste("O3 > 70   (", round(length(SPAN_span$o3)/length(temp_L00_ThisMonth$o3)*100,2),"%)")),col="darkred" ,pos=1,cex = 1.8)
  segments(head(temp_L00_ThisMonth$date), 70, tail(temp_L00_ThisMonth$date), 70, lty = 2, col="pink",lwd = 1.75) 
  text(temp_L00_ThisMonth$date[1], 18, labels = c(paste("O3 < 10   (", round(length(SPAN_zero$o3)/length(temp_L00_ThisMonth$o3)*100,2),"%)")),col="darkblue",pos=1,cex = 1.8)
  segments(head(temp_L00_ThisMonth$date), 10, tail(temp_L00_ThisMonth$date), 10, lty = 2, col="darkblue",lwd = 1.75)
  
  legend("topleft", legend=c("Span","Zero"), pch =c(25,24),  col=c("darkred","darkblue"), cex = 2.5)
  
  #
  # -------------------------------------------------------------------------------------------
  # -------------------------------------------------------------------------------------------
  # Plotting Bench_T e Lamp_T
  #
  temp_L01_ThisMonth$Bench_T[temp_L01_ThisMonth$Bench_T > 900] <- 0
  temp_L01_ThisMonth$Lamp_T [temp_L01_ThisMonth$Lamp_T  > 900] <- 0
  
  plot(ylim=c(mean(temp_L01_ThisMonth$Bench_T)-20,mean(temp_L01_ThisMonth$Lamp_T)+20),
       xlim = c(min(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
       mgp = c(8, 4, 0),
       temp_L01_ThisMonth$date, temp_L01_ThisMonth$Lamp_T, type = "h",
       xlab = "",ylab =("Lamp T & Bench T - L01"), col="pink", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  lines(temp_L01_ThisMonth$date, temp_L01_ThisMonth$Bench_T, type = "l", lty = 1, col="blue",lwd = 0.5)
  title(paste("Lamp T and Bench T - L01 -",questo_anno,This_Month_Name), line = -3)
  
  xxB <- c(temp_L01_ThisMonth$date, rev(temp_L01_ThisMonth$date))
  yyB <- c(rep(0, nrow(temp_L01_ThisMonth)), rev(temp_L01_ThisMonth$Bench_T))
  polygon(xxB, yyB, col='gold',border = "gold")
  
  lines(temp_L01_ThisMonth$date, temp_L01_ThisMonth$Lamp_T, type = "l", lty = 1, col="darkred",lwd = 2)
  lines(temp_L01_ThisMonth$date, temp_L01_ThisMonth$Bench_T, type = "l", lty = 1, col="darkblue",lwd = 2)
  
  legend("topleft", legend=c("Lamp_T","Bench_T"), lty=c(1,1), lwd = c(2,2),  col=c("darkred","darkblue"), cex = 2.5)
    #
  # -------------------------------------------------------------------------------------------
  # -------------------------------------------------------------------------------------------
  # Plotting PLOT Int_A e Int_B
  #
  temp_L01_ThisMonth$Int_A[temp_L01_ThisMonth$Int_A > 900000] <- 0
  temp_L01_ThisMonth$Int_B[temp_L01_ThisMonth$Int_B > 900000] <- 0
  
  plot(ylim=c(mean(temp_L01_ThisMonth$Int_A)-4000,mean(temp_L01_ThisMonth$Int_B)+4000),
       xlim = c(min(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(OZO_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
       mgp = c(8, 4, 0),
       temp_L01_ThisMonth$date, temp_L01_ThisMonth$Int_B, type = "h",
       xlab = "",ylab =("Int_A & Int_B - L01"), col="seashell", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  lines(temp_L01_ThisMonth$date, temp_L01_ThisMonth$RH, type = "l", lty = 1, col="mediumorchid4",lwd = 0.5)
  title(paste("Int_A and Int_B - L01 -",questo_anno,This_Month_Name), line = -3)
  
  xxI <- c(temp_L01_ThisMonth$date, rev(temp_L01_ThisMonth$date))
  yyI <- c(rep(0, nrow(temp_L01_ThisMonth)), rev(temp_L01_ThisMonth$Int_A))
  polygon(xxI, yyI, col='salmon',border = "salmon")
  
  lines(temp_L01_ThisMonth$date, temp_L01_ThisMonth$Int_A, type = "l", lty = 1, col="darkblue",lwd = 2)
  lines(temp_L01_ThisMonth$date, temp_L01_ThisMonth$Int_B, type = "l", lty = 1, col="darkred",lwd = 2)
  
  legend("topleft", legend=c("Int_A","Int_B"), lty=c(1,1), lwd = c(2,2),  col=c("darkblue","darkred"), cex = 2.5)
  ...
  text(x = 0, y = 0.55, paste("\n", "O3 min: (", round(min(mensile$o3),1),") - O3 max: (", round(max(mensile$o3),1),") - O3 mean: (", round(mean(mensile$o3),1),") - O3 sd: (", round(sd(mensile$o3),1),")","\n",
                              "O3 percentile:    5th: (",round(quantile(mensile$o3, probs = c(0.05)),digits=1),")",
                              " - 25th: (",round(quantile(mensile$o3, probs = c(0.25)),digits=1),")",
                              " - 50th: (",round(quantile(mensile$o3, probs = c(0.50)),digits=1),")",
                              " - 75th: (",round(quantile(mensile$o3, probs = c(0.75)),digits=1),")",
                              " - 95th: (",round(quantile(mensile$o3, probs = c(0.95)),digits=1),")","\n","\n",
                              "L02 numflag = 0.999 (number of days of the year):   ", FAIL_DAYS,
...
# First semester
#
Fhalf           <-subset(OZO_L02_today_Y,monthNum<7)
...
if (nrow(Fhalf)==0){ } else 
{
  ...
  {
    print(i)
    mensile            <-subset(OZO_L02_today_Y, month == i)
    mensile$giorno     <-format(mensile$date,"%d")
    mensile$mean          <-mean(mensile$o3)
    mensile$sd            <-sd(mensile$o3)    
    men_MIN               <-subset(mensile, o3 == min(mensile$o3))
    men_MAX               <-subset(mensile, o3 == max(mensile$o3))
    {
      plot(ylim=c(1,110),
           mensile$date, mensile$o3, type = "h",
           xlab = "",
           ylab =bquote(paste("O"[3], " (nmol/mol) - L02")), 
           col="lightblue", 
           panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
      title(main=paste(i,questo_anno,sep=" "), 
            line = -2.2)
      
      title(paste("Statistics: min: (", round(min(mensile$o3),1),") - max: (", round(max(mensile$o3),1),") - mean: (", round(mean(mensile$o3),1),") - sd: (", round(sd(mensile$o3),1),")")
            ,col="black",cex.main = 1.2, line = -3.8, font.main = 1)
      
      lines(mensile$date, mensile$o3, type = "l", lty = 1, col="darkred",lwd = 2)
      
      points(men_MAX$date[1], men_MAX$o3[1], col= "magenta",pch=20)
      text(men_MAX$date[1], -0.5 , labels = paste("Max (",format(men_MAX$date[1],"%B %d"),")",sep=""),col="magenta",pos=3)
      segments(men_MAX$date[1], men_MAX$o3[1], men_MAX$date[1], -1, lty = 2, col="black",lwd = 1)

      points(men_MIN$date[1], men_MIN$o3[1], col= "blue",pch=20)
      text(men_MIN$date[1], -0.5 , labels = paste("Min (",format(men_MIN$date[1],"%B %d"),")",sep=""),col="blue",pos=3)
      segments(men_MIN$date[1], men_MIN$o3[1], men_MIN$date[1], -1, lty = 2, col="black",lwd = 1)
    }   
  }
  dev.off() 
...
# Second semester
#
Shalf<-subset(OZO_L02_today_Y,monthNum>6)
...
#
OZO_L02_today_Y$season[OZO_L02_today_Y$monthNum>=1  & OZO_L02_today_Y$monthNum<=3]     <- 1
OZO_L02_today_Y$season[OZO_L02_today_Y$monthNum>=4  & OZO_L02_today_Y$monthNum<=6]     <- 2
OZO_L02_today_Y$season[OZO_L02_today_Y$monthNum>=7  & OZO_L02_today_Y$monthNum<=9]     <- 3
OZO_L02_today_Y$season[OZO_L02_today_Y$monthNum>=10 & OZO_L02_today_Y$monthNum<=12]    <- 4
#
# -------------------------------------------------------------------------------------------
...
seasons<-c(OZO_L02_today_Y[!duplicated(OZO_L02_today_Y[,c('season')]),]$season)
seasons 
# -------------------------------------------------------------------------------------------
# Preparing plotting files
#  
...
for (stg in seasons)
{
  ...
  OZO_L02_stg      <-subset(OZO_L02_today_Y, season == stg)
  ...
  stagionale<-subset(OZO_L02_stg, numflag != 0.999)
  ...
  stagionale$mean      <- mean(stagionale$o3)
  stagionale$sd        <- sd(stagionale$o3)
  stg_MIN              <- subset(stagionale, o3 == min(stagionale$o3))
  stg_MAX              <- subset(stagionale, o3 == max(stagionale$o3))
  ...
  #
  plot(ylim=c(-0.1,100),
       stagionale$date, stagionale$o3, type = "h",
       mgp = c(7, 3, 0),
       xlab = "",ylab =bquote(paste("O"[3], " (nmol/mol) - L02")), 
       col="lightblue", 
       panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 0.5))
  lines(stagionale$date, stagionale$o3, type = "l", lty = 1, col="darkred",lwd = 2)
  title(main=bquote(bold(paste("O"[3], " - L02 - ",.(questo_anno)," ",.(stg_name)))),
        line = -4.5)
  
  mtext(paste("Observations from", OBS_stg_start," to ", 
              OBS_stg_end,"(",1+(as.integer(tail((OZO_L02_stg$date),1)-head((OZO_L02_stg$date)),1))," days)       -       Statistics: ","  O3 min: (", min(stagionale$o3),") - O3 max: (", max(stagionale$o3),") - O3 mean: (", round(mean(stagionale$o3),1),") - O3 sd: (", round(sd(stagionale$o3),1),")"),
                     col="black",cex = 1.7, line = -8.0, font.main = 1)
  
  points(stg_MAX$date[1], stg_MAX$o3[1], col= "magenta",pch=20)
  segments(stg_MAX$date[1], stg_MAX$o3[1], stg_MAX$date[1], stg_MIN$o3[1]-0.18, lty = 2, col="black",lwd = 1)
  text(stg_MAX$date[1], 0, labels = paste("Max","(",(format(stg_MAX$date[1],"%B %d")),")"),
       col="magenta",
       pos=3,
       cex = 1.6)
  
  points(stg_MIN$date[1], stg_MIN$o3[1], col= "blue",pch=20)
  segments(stg_MIN$date[1], stg_MIN$o3[1], stg_MIN$date[1], stg_MIN$o3[1]-0.18, lty = 2, col="black",lwd = 1)
  text(stg_MIN$date[1], 0, labels = paste("Min","(",(format(stg_MIN$date[1],"%B %d")),")"),
       col="blue",
       pos=3,
       cex = 1.7)
  #
...
# Defining statistics
#
OZO_L02_today_Y$mean    <-mean(OZO_L02_today_Y$o3)
Y_MIN                   <-subset(OZO_L02_today_Y, o3 == min(OZO_L02_today_Y$o3))
Y_MAX                   <-subset(OZO_L02_today_Y, o3 == max(OZO_L02_today_Y$o3))
...
#
plot(ylim=c(-0.1,100),
     xlim = c(min(c(OZO_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(OZO_L02_today_Y$date,temp_L01$date,temp_L00$date))),
     mgp = c(8, 4, 0),
     OZO_L02_today_Y$date, OZO_L02_today_Y$o3, type = "h",
     xlab = "",ylab =bquote(paste("O"[3], " (nmol/mol) - L02")), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
title(bquote(bold(paste("O"[3], " - L02 -",.(questo_anno)))), line = -3)
lines(OZO_L02_today_Y$date, OZO_L02_today_Y$o3, type = "l", lty = 1, col="darkred",lwd = 2)

points(Y_MAX$date[1], Y_MAX$o3[1], col= "magenta",pch=20)
text(Y_MAX$date[1], -0.1, labels = paste0("Max (",format(Y_MAX$date[1],"%B %d"),")"),col="magenta",pos=3,cex = 1.8)
segments(Y_MAX$date[1], Y_MAX$o3[1], Y_MAX$date[1], -1, lty = 2, col="black",lwd = 1)

points(Y_MIN$date[1], Y_MIN$o3[1], col= "blue",pch=20)
text(Y_MIN$date[1], -0.1, labels = paste0("Min (",format(Y_MIN$date[1],"%B %d"),")"),col="blue",pos=3,cex = 1.8)
segments(Y_MIN$date[1], Y_MIN$o3[1], Y_MIN$date[1], -1, lty = 2, col="black",lwd = 1)

lines(OZO_L02_today_Y$date,OZO_L02_today_Y$mean, type = "l", lty = 1, col="red",lwd = 1)
text(OZO_L02_today_Y$date[1],OZO_L02_today_Y$mean[1]+0.05, labels = c("Mean"),pos=2,col="red",cex = 1.5)
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
# Plotting NUMFLAG L01
...
# Plotting Flow_A e Flow_B
#
plot(ylim=c(min(OZO_L01_today_Y$Flow_A)-0.25,max(OZO_L01_today_Y$Flow_A)+0.25),
     xlim = c(min(c(OZO_L02_today$date,temp_L01$date,temp_L00$date)),max(c(OZO_L02_today$date,temp_L01$date,temp_L00$date))),
     mgp = c(8, 4, 0),
     OZO_L01_today_Y$date, OZO_L01_today_Y$Flow_A, type = "l", lwd = 0.5,
     xlab = "",ylab =("Flow_A & Flow_B - L01"), 
     col="darkred", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
title(paste("Flow_A and Flow_B - L01 -",questo_anno), line = -3)

lines(OZO_L01_today_Y$date, OZO_L01_today_Y$Flow_A, type = "l", lty = 1, col="darkred",lwd = 1.5)
lines(OZO_L01_today_Y$date, OZO_L01_today_Y$Flow_B, type = "l", lty = 1, col="darkblue",lwd = 0.5)

segments(head(temp_L01$date), 0.35, tail(temp_L01$date), 0.35, lty = 6, col="red",lwd = 0.5)
text(temp_L01$date[1], 0.32, labels = c("Warning threshold (Flow 0.35)"),col="red",pos=4,cex = 1.8)
...
# -------------------------------------------------------------------------------------------
# Plotting span  
SPAN_span<-subset(temp_L00,(temp_L00$numflag==0.682000000 & temp_L00$o3>70))
SPAN_zero<-subset(temp_L00,(temp_L00$numflag==0.682000000 & temp_L00$o3<10))
...
plot(ylim=c(-10,150),
     xlim = c(min(c(OZO_L02_today$date,temp_L01$date,temp_L00$date)),max(c(OZO_L02_today$date,temp_L01$date,temp_L00$date))),
     mgp = c(8, 4, 0),
     temp_L00$date, temp_L00$o3, type = "n",
     xlab = "",ylab =("Span O3 - L00"), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
title(paste("Span O3 - L00 -",questo_anno), line = -3)

points(xx_S, yy_S, col='darkred' ,pch = 25, cex = 2.5)
points(xx_z, yy_z, col='darkblue',pch = 24, cex = 2.5)

text(temp_L00$date[1], 67, labels = c(paste("O3 > 70   (", round(length(SPAN_span$o3)/length(temp_L00$o3)*100,2),"%)")),col="darkred",pos=1,cex = 1.8)
segments(head(temp_L00$date), 70, tail(temp_L00$date), 70, lty = 2, col="pink",lwd = 0.75) 
text(temp_L00$date[1], 18, labels = c(paste("O3 < 10   (", round(length(SPAN_zero$o3)/length(temp_L00$o3)*100,2),"%)")),col="darkblue",pos=1,cex = 1.8)
segments(head(temp_L00$date), 10, tail(temp_L00$date), 10, lty = 2, col="darkblue",lwd = 0.75)

legend("topleft", legend=c("Span","Zero"), pch =c(25,24),  col=c("darkred","darkblue"), cex = 2.5)
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
# Plotting Bench_T and Lamp_T
#
xxB <- c(OZO_L01_today_Y$date, rev(OZO_L01_today_Y$date))
yyB <- c(rep(0, nrow(OZO_L01_today_Y)), rev(OZO_L01_today_Y$Bench_T))

plot(ylim=c(mean(OZO_L01_today_Y$Bench_T)-20,mean(OZO_L01_today_Y$Lamp_T)+10),
     xlim = c(min(c(OZO_L01_today_Y$date,temp_L01$date,temp_L00$date)),max(c(OZO_L01_today_Y$date,temp_L01$date,temp_L00$date))),
     mgp = c(8, 4, 0),
     OZO_L01_today_Y$date, OZO_L01_today_Y$Lamp_T, type = "h",
     xlab = "",ylab =("Lamp T & Bench T - L01"), col="lightyellow", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
title(paste("Lamp T and Bench T - L01 -",questo_anno), line = -3)

polygon(xxB, yyB, col='gold',border = "gold")
lines(OZO_L01_today_Y$date, OZO_L01_today_Y$Lamp_T, type = "l", lty = 1, col="darkred",lwd = 2)
lines(OZO_L01_today_Y$date, OZO_L01_today_Y$Bench_T, type = "l", lty = 1, col="darkblue",lwd = 2)

legend("topleft", legend=c("Lamp_T","Bench_T"), lty=c(1,1), lwd = c(2,2),  col=c("darkred","darkblue"), cex = 2.5)
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
# Plotting Int_A and Int_B
#
xx <- c(OZO_L01_today_Y$date, rev(OZO_L01_today_Y$date))
yy <- c(rep(0, nrow(OZO_L01_today_Y)), rev(OZO_L01_today_Y$Int_A))

plot(ylim=c(min(OZO_L01_today_Y$Int_A)-5000,max(OZO_L01_today_Y$Int_B)+5000),
     mgp = c(8, 4, 0),
     xlim = c(min(c(OZO_L02_today$date,temp_L01$date,temp_L00$date)),max(c(OZO_L02_today$date,temp_L01$date,temp_L00$date))),OZO_L01_today_Y$date, OZO_L01_today_Y$Int_B, type = "h",
     xlab = "",ylab =("Int_A & Int_B - L01"), col="pink", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
title(paste("Int_A and Int_B - L01 -",questo_anno), line = -3)

polygon(xx, yy, col='salmon',border = "salmon")
lines(OZO_L01_today_Y$date, OZO_L01_today_Y$Int_A, type = "l", lty = 1, col="darkblue",lwd = 2)
lines(OZO_L01_today_Y$date, OZO_L01_today_Y$Int_B, type = "l", lty = 1, col="darkred",lwd = 2)
...
text(x = -0.005, y = 0.75, paste(" Observations from", OBS_start," to ", OBS_end,
                                 "\n",
                                 "(days of observation: ", 1+(as.integer(tail((OZO_L02_today_Y$date),1)-head((OZO_L02_today_Y$date)),1)),")")
     , cex = 2.8, col = "black",pos = 4)
...
text(x = 0, y = 0.50, paste("\n", "O3 min: (", round(min(OZO_L02_today_Y$o3),1),") - O3 max: (", round(max(OZO_L02_today_Y$o3),1),") - O3 mean: (", round(mean(OZO_L02_today_Y$o3),1),") - O3 sd: (", round(sd(OZO_L02_today_Y$o3),1),")",
                            "\n",
                            "O3 percentile:    5th: (",round(quantile(OZO_L02_today_Y$o3, probs = c(0.05)),digits=1),")",
                            " - 25th: (",round(quantile(OZO_L02_today_Y$o3, probs = c(0.25)),digits=1),")",
                            " - 50th: (",round(quantile(OZO_L02_today_Y$o3, probs = c(0.50)),digits=1),")",
                            " - 75th: (",round(quantile(OZO_L02_today_Y$o3, probs = c(0.75)),digits=1),")",
                            " - 95th: (",round(quantile(OZO_L02_today_Y$o3, probs = c(0.95)),digits=1),")","\n","\n",
...
calendarPlot(OZO_L02_today_Y, pollutant = "o3", year = questo_anno, month=c(1:12), auto.text = TRUE,
             key.footer = "O3 (nmol/mol)", key.position = "right", key = TRUE,
             main = paste("O3 in",questo_anno))
dev.off()
##                                          # END PART 3.4 #
###########################################################################################################################
...
#
mydata                    <-data.frame(OZO_L02_today_Y$date,OZO_L02_today_Y$o3)
colnames(mydata)          <- c("date","o3")
#
# -------------------------------------------------------------------------------------------
...
TV_ANNUAL_H               <-timeVariation(mydata, pollutant = "o3", normalise = FALSE, 
                                          ylab = paste("O3 (nmol/mol) -",questo_anno),
...
plot(0:297, ty="n",xaxt = "n",yaxt = "n",xlab = paste("O3 -",questo_anno,"- annual trend analysis",sep=" "),ylab = "")
#
# -------------------------------------------------------------------------------------------
# Specifing the position of the image through bottom-left and top-right coords
#
...
#
o3_hh                 <- timeAverage(mydata, avg.time = "hour", data.thresh = 75, statistic = "mean",
                                         start.date = mydata$date[1], 
                                         end.date = mydata$date[length(mydata)], 
                                         interval = "hour")

o3_tv                 <- timeVariation(o3_hh, pollutant = "o3", type = "season", conf.int = 0.95,
...
PLOT_TVN1_NAME            <-paste("tmp_s1_o3_",questo_anno,"_TIMEVARIATION_GRAPH_SEASONAL_",gsub("-","",Sys.Date()),sep = "")
...
PLOT_TVN2_NAME            <-paste("tmp_s2_o3_",questo_anno,"_TIMEVARIATION_GRAPH_SEASONAL_",gsub("-","",Sys.Date()),sep = "")
...
plot(0:297, ty="n",xaxt = "n",yaxt = "n",xlab = paste("O3 - ",questo_anno," - trend analysis by season",sep=""),ylab = "")
...
# Cleaning Destination directory
#
unico_T                   <-c(OZO_L02_today_Y[!duplicated(OZO_L02_today_Y[,c('monthNum')]),]$monthNum)
...
{ print(qm)
  ...
  # Creating data subset
  #
  OZO_L02_ThisMonth       <-subset(OZO_L02_today, as.integer(monthNum)==qm)
  temp_L02_ThisMonth      <-subset(temp_L02, as.integer(monthNum)==qm)
  temp_L00_ThisMonth      <-subset(temp_L00, as.integer(monthNum)==qm)
  ...
  mydata                  <- data.frame(mensile$date,mensile$o3)
  colnames(mydata)        <- c("date","o3")
  ...
  myOutput                <-timeVariation(mydata, pollutant = "o3", 
                                          ylab = paste("O3 (nmol/mol) -",questo_anno), 
                                          type="season",
  ...
  TV_ANNUAL_H             <-timeVariation(mydata, pollutant = "o3", ylab = paste("O3 (nmol/mol) -",questo_anno),
  ...
  plot(0:297, ty="n",xaxt = "n",yaxt = "n",
       xlab = paste("O3 - ",This_Month_Name," ",questo_anno," - monthly trend analysis",sep=""),ylab = "")
  #
  # -------------------------------------------------------------------------------------------
