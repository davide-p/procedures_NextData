...
##_______________________________________________________________________________________________________________________##
## Script filename: CPC_P22_1810.R                                                                                       ##
...
#
CPC_L02_today           <-data.frame(temp_L02$date,temp_L02$conc,temp_L02$numflag)
#
colnames(CPC_L02_today) <- c("date","conc","numflag")
#
CPC_L02_today$month     <- format(CPC_L02_today$date,"%B")
CPC_L02_today$monthNum  <- as.numeric(format(CPC_L02_today$date,"%m"))
CPC_L02_today$day       <- as.numeric(format(CPC_L02_today$date,"%d"))
#
CPC_FAIL_today          <- subset(CPC_L02_today, numflag == 0.999)
#
TFAIL                   <- subset(CPC_FAIL_today, select=c("month","day","monthNum","numflag"))
...
if (nrow(mesi_FAIL) == 0)  { }
#
CPC_L02_today_Y         <- subset(CPC_L02_today, numflag != 0.999)
#
unico                   <- c(CPC_L02_today_Y[!duplicated(CPC_L02_today_Y[,c('month')]),]$month)
#
# -------------------------------------------------------------------------------------------
#
##                                         # END PART 2.0 #
...
#
unico_M             <- c(CPC_L02_today_Y[!duplicated(CPC_L02_today_Y[,c('monthNum')]),]$monthNum)

for (qm in unico_M)
{
  CPC_L02_ThisMonth    <-subset(CPC_L02_today_Y, as.integer(monthNum)==qm)
  ...
  #
  FAIL_ThisMonth      <- subset(CPC_FAIL_today, as.integer(monthNum) == qm)
  ...
  mensile             <-subset(CPC_L02_ThisMonth, numflag != 0.999)
  ...
  #
  mensile$mean        <-mean(mensile$conc)
  mensile$sd          <-sd(mensile$conc)
  men_MIN             <-subset(mensile, conc == min(mensile$conc))
  men_MAX             <-subset(mensile, conc == max(mensile$conc))
  # -------------------------------------------------------------------------------------------
  # Cleaning Destination directory
  # 
  ...
  #
  m <- rbind(c(1, 1), c(2, 2), c(3, 3), c(4, 4), c(5, 5), c(6, 7))#, c(7, 8))
  layout(m)
  #
  # -------------------------------------------------------------------------------------------
  # -------------------------------------------------------------------------------------------
  # Plotting CONC
  #
  plot(ylim=c(men_MIN$conc[1]-300,men_MAX$conc[1]+300),
       xlim = c(min(c(CPC_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(CPC_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
        mgp = c(8, 4, 0),
       mensile$date, mensile$conc, type = "h",
       xlab = "",ylab =expression(paste("Ntot (#/cm"^{3},") - L02")), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  title(paste("Ntot - L02 -",questo_anno,This_Month_Name), line = -3)
  lines(CPC_L02_ThisMonth$date, mensile$conc, type = "l", lty = 1, col="darkred",lwd = 2)
  
  points(men_MAX$date[1], men_MAX$conc[1], col= "magenta",pch=20)
  text(men_MAX$date[1], men_MAX$conc[1], labels = paste("Max (", format(men_MAX$date[1],"%B %d"), ")", sep=""),col="magenta",pos=3,cex = 1.8)
  segments(men_MAX$date[1], men_MAX$conc[1], men_MAX$date[1], men_MIN$conc[1]-400, lty = 2, col="black",lwd = 1)

  points(men_MIN$date[1], men_MIN$conc[1], col= "blue",pch=20)
  text(men_MIN$date[1], men_MIN$conc[1], labels = paste("Min (", format(men_MIN$date[1],"%B %d"), ")", sep =""),col="blue",pos=1,cex = 1.8)
  segments(men_MIN$date[1], men_MIN$conc[1], men_MIN$date[1], men_MIN$conc[1]-400, lty = 2, col="black",lwd = 1)
  ...
  text(x = 0, y = 0.55, paste("\n", "Ntot min: (", round(min(mensile$conc),0),") - Ntot max: (", round(max(mensile$conc),0),") - Ntot mean: (", round(mean(mensile$conc),0),") - Ntot sd: (", round(sd(mensile$conc),0),")","\n",
                              "Ntot percentile:    5th: (",round(quantile(mensile$conc, probs = c(0.05)),digits=0),")",
                              " - 25th: (",round(quantile(mensile$conc, probs = c(0.25)),digits=0),")",
                              " - 50th: (",round(quantile(mensile$conc, probs = c(0.50)),digits=0),")",
                              " - 75th: (",round(quantile(mensile$conc, probs = c(0.75)),digits=0),")",
                              " - 95th: (",round(quantile(mensile$conc, probs = c(0.95)),digits=0),")","\n","\n",
...
# First semester
#
Fhalf           <-subset(CPC_L02_today_Y,monthNum<7)
...
if (nrow(Fhalf)==0){ } else 
{
  ...
  {
    print(i)
    mensile            <-subset(CPC_L02_today_Y, month == i)
    mensile$giorno     <-format(mensile$date,"%d")
    mensile$mean       <-mean(mensile$conc)
    mensile$sd         <-sd(mensile$conc)    
    men_MIN            <-subset(mensile, conc == min(mensile$conc))
    men_MAX            <-subset(mensile, conc == max(mensile$conc))
    {
      plot(ylim=c(men_MIN$conc[1]-500,men_MAX$conc[1]+500),
           mensile$date, mensile$conc, type = "h",
           xlab = "",ylab =expression(paste("Ntot (#/cm"^{3},") - L02")), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
      title(main=paste(i,questo_anno,sep=" "), 
            line = -2.2)
      
      title(paste("Statistics: min: (", round(min(mensile$conc),0),") - max: (", round(max(mensile$conc),0),") - mean: (", round(mean(mensile$conc),0),") - sd: (", round(sd(mensile$conc),0),")")
            ,col="black",cex.main = 1.2, line = -3.8, font.main = 1)
      
      lines(mensile$date, mensile$conc, type = "l", lty = 1, col="darkred",lwd = 2)
      
      points(men_MAX$date[1], men_MAX$conc[1], col= "magenta",pch=20)
      text(men_MAX$date[1], men_MAX$conc[1], labels = paste("Max (",format(men_MAX$date[1],"%B %d"),")",sep=""),col="magenta",pos=3)
      segments(men_MAX$date[1], men_MAX$conc[1], men_MAX$date[1], men_MIN$conc[1]-400, lty = 2, col="black",lwd = 1)

      points(men_MIN$date[1], men_MIN$conc[1], col= "blue",pch=20)
      text(men_MIN$date[1], men_MIN$conc[1], labels = paste("Min (",format(men_MIN$date[1],"%B %d"),")",sep=""),col="blue",pos=1)
      segments(men_MIN$date[1], men_MIN$conc[1], men_MIN$date[1], men_MIN$conc[1]-400, lty = 2, col="black",lwd = 1)
    }   
  }
  dev.off() 
...
# Second semester
#
Shalf<-subset(CPC_L02_today_Y,monthNum>6)
if (nrow(Shalf)==0){}else 
{
  # -------------------------------------------------------------------------------------------
  # Cleaning Destination directory
  # 
  FILE_PLOT_2HM<-list.files(path = PLOT_DIR_M, pattern = paste(s_GAW_ID, inst_type,questo_anno,"SEMESTER_1st_GRAPH_*",sep = "_"), all.files = FALSE,
                            full.names = F, recursive = FALSE,
                            ignore.case = FALSE, include.dirs = F, no.. = FALSE)
  FILE_PLOT_2HM
  ...
  # Preparing plotting parameters (font size, margins, ...)
  # 
  png(file=paste(PLOT_DIR_M,paste(s_GAW_ID,"_",inst_type,"_",questo_anno,"_SEMESTER_2st_GRAPH_",gsub("-","",Sys.Date()),".png",sep = ""),sep = "/"),
      width = 2480, height = 3508)
  #
  # -------------------------------------------------------------------------------------------
  # Creating the plotting matrix
...
#
CPC_L02_today_Y$season[CPC_L02_today_Y$monthNum>=1  & CPC_L02_today_Y$monthNum<=3]      <- 1
CPC_L02_today_Y$season[CPC_L02_today_Y$monthNum>=4  & CPC_L02_today_Y$monthNum<=6]      <- 2
CPC_L02_today_Y$season[CPC_L02_today_Y$monthNum>=7  & CPC_L02_today_Y$monthNum<=9]      <- 3
CPC_L02_today_Y$season[CPC_L02_today_Y$monthNum>=10 & CPC_L02_today_Y$monthNum<=12]     <- 4
#
# -------------------------------------------------------------------------------------------
...
seasons<-c(CPC_L02_today_Y[!duplicated(CPC_L02_today_Y[,c('season')]),]$season)
seasons 
# -------------------------------------------------------------------------------------------
# Preparing plotting files
#  
...
for (stg in seasons)
{
  ...
  CPC_L02_stg      <-subset(CPC_L02_today_Y, season == stg)
  ...
  stagionale<-subset(CPC_L02_stg, numflag != 0.999)
  ...
  stagionale$mean      <- mean(stagionale$conc)
  stagionale$sd        <- sd(stagionale$conc)
  stg_MIN              <- subset(stagionale, conc == min(stagionale$conc))
  stg_MAX              <- subset(stagionale, conc == max(stagionale$conc))
  ...
  # Plotting conc
  #
  plot(ylim=c(stg_MIN$conc[1]-700,stg_MAX$conc[1]+500),
       stagionale$date, stagionale$conc, type = "h",
       mgp = c(7, 3, 0),
       xlab = "",ylab =expression(paste("Ntot (#/cm"^{3},") - L02")), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 0.5))
  lines(stagionale$date, stagionale$conc, type = "l", lty = 1, col="darkred",lwd = 1)
  title(main=paste("Ntot",questo_anno,"   ",stg_name), line = -4.5)
  
  mtext(paste("Observations from", OBS_stg_start,
                           " to ", OBS_stg_end,"(",1+(as.integer(tail((CPC_L02_stg$date),1)-head((CPC_L02_stg$date)),1)),"days)       -       Statistics: ",
                           "  Ntot min: (", round(min(stagionale$conc),0),") - Ntot max: (", round(max(stagionale$conc),0),") - Ntot mean: (", round(mean(stagionale$conc),0),") - Ntot sd: (", round(sd(stagionale$conc),0),")"),
                     col="black",cex = 1.7, line = -8.0, font.main = 1)
  
  points(stg_MAX$date[1], stg_MAX$conc[1], col= "magenta",pch=20)
  segments(stg_MAX$date[1], stg_MAX$conc[1], stg_MAX$date[1], stg_MIN$conc[1]-600, lty = 2, col="black",lwd = 1)
  text(stg_MAX$date[1], stg_MIN$conc[1]-500, labels = paste("Max","(",(format(stg_MAX$date[1],"%B %d")),")"),
       col="magenta",
       pos=1,
       cex = 1.6)
  
  points(stg_MIN$date[1], stg_MIN$conc[1], col= "blue",pch=20)
  segments(stg_MIN$date[1], stg_MIN$conc[1], stg_MIN$date[1], stg_MIN$conc[1]-600, lty = 2, col="black",lwd = 1)
  text(stg_MIN$date[1], stg_MIN$conc[1]-500, labels = paste("Min","(",(format(stg_MIN$date[1],"%B %d")),")"),
       col="blue",
       pos=1,
       cex = 1.6)
  #
...
# Defining statistics
#
CPC_L02_today_Y$mean    <-mean(CPC_L02_today_Y$conc)
Y_MIN                   <-subset(CPC_L02_today_Y, conc == min(CPC_L02_today_Y$conc))
Y_MAX                   <-subset(CPC_L02_today_Y, conc == max(CPC_L02_today_Y$conc))
...
# Crating the plotting matrix
#
m <- rbind(c(1, 1), c(2, 2), c(3, 3), c(4, 4), c(5, 5), c(6, 7))
layout(m)
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
# Plotting CONC
#
plot(ylim=c(Y_MIN$conc[1]-300,Y_MAX$conc[1]+300),
     xlim = c(min(c(CPC_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(CPC_L02_today_Y$date,temp_L01$date,temp_L00$date))),
     mgp = c(8, 4, 0),
     CPC_L02_today_Y$date, CPC_L02_today_Y$conc, type = "h",
     xlab = "",ylab =expression(paste("Ntot (#/cm"^{3},") - L02")), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
title(paste("Ntot - L02 -",questo_anno), line = -3.5)
lines(CPC_L02_today_Y$date, CPC_L02_today_Y$conc, type = "l", lty = 1, col="darkred",lwd = 0.5)

points(Y_MAX$date[1], Y_MAX$conc[1], col= "magenta",pch=20)
text(Y_MAX$date[1], Y_MAX$conc[1], labels = paste0("Max (",format(Y_MAX$date[1],"%B %d"),")"),col="magenta",pos=3,cex = 1.8)
segments(Y_MAX$date[1], Y_MAX$conc[1], Y_MAX$date[1], Y_MIN$conc[1]-400, lty = 2, col="black",lwd = 1)

points(Y_MIN$date[1], Y_MIN$conc[1], col= "blue",pch=20)
text(Y_MIN$date[1], Y_MIN$conc[1], labels = paste0("Min (",format(Y_MIN$date[1],"%B %d"),")"),col="blue",pos=1,cex = 1.8)
segments(Y_MIN$date[1], Y_MIN$conc[1], Y_MIN$date[1], Y_MIN$conc[1]-400, lty = 2, col="black",lwd = 1)

lines(CPC_L02_today_Y$date,CPC_L02_today_Y$mean, type = "l", lty = 1, col="red",lwd = 1)
text(CPC_L02_today_Y$date[1],CPC_L02_today_Y$mean[1]+200, labels = c("Mean"),pos=2,col="red",cex = 1.5)
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
# Plotting NUMFLAG L01
...
text(x = -0.005, y = 0.75, paste(" Observations from", OBS_start," to ", OBS_end,
                                 "\n",
                                 "(days of observation: ", 1+(as.integer(tail((CPC_L02_today_Y$date),1)-head((CPC_L02_today_Y$date)),1)),")")
     , cex = 2.8, col = "black",pos = 4)
...
if(is.null(mesi_FAIL_COUNT$print)){
  text(x = 0, y = 0.50, paste("\n", "Ntot min: (", round(min(CPC_L02_today_Y$conc),0),") - Ntot max: (", round(max(CPC_L02_today_Y$conc),0),") - Ntot mean: (", round(mean(CPC_L02_today_Y$conc),0),") - Ntot sd: (", round(sd(CPC_L02_today_Y$conc),0),")",
                              "\n",
                              "Ntot percentile:    5th: (",round(quantile(CPC_L02_today_Y$conc, probs = c(0.05)),digits=0),")",
                              " - 25th: (",round(quantile(CPC_L02_today_Y$conc, probs = c(0.25)),digits=0),")",
                              " - 50th: (",round(quantile(CPC_L02_today_Y$conc, probs = c(0.50)),digits=0),")",
                              " - 75th: (",round(quantile(CPC_L02_today_Y$conc, probs = c(0.75)),digits=0),")",
                              " - 95th: (",round(quantile(CPC_L02_today_Y$conc, probs = c(0.95)),digits=0),")","\n","\n",
...
#
calendar_table <- CPC_L02_today_Y

perc <- quantile(calendar_table$conc, c(0.05, 0.95))

calendar_table$conc[calendar_table$conc < min(perc)] <- min(perc)
calendar_table$conc[calendar_table$conc > max(perc)] <- max(perc)
...
calendarPlot(calendar_table, pollutant = "conc", year = questo_anno, month=c(1:12), auto.text = TRUE,
             key.footer = expression(paste("Ntot (#/cm"^{3},")")), key.position = "right", key = TRUE,
             main = paste("Ntot in",questo_anno))
dev.off()
##                                          # END PART 3.4 #
###########################################################################################################################
...
#
mydata                    <-data.frame(CPC_L02_today_Y$date,CPC_L02_today_Y$conc)
colnames(mydata)          <- c("date","conc")
#
# -------------------------------------------------------------------------------------------
...
TV_ANNUAL_H               <-timeVariation(mydata, pollutant = "conc", normalise = FALSE, 
                                          ylab = paste("Ntot (#/cm3) -",questo_anno),
...
plot(0:297, ty="n",xaxt = "n",yaxt = "n",xlab = paste("Ntot -",questo_anno,"- annual trend analysis",sep=" "),ylab = "")
#
# -------------------------------------------------------------------------------------------
...
# Cleaning Destination directory
#
unico_T                   <-c(CPC_L02_today_Y[!duplicated(CPC_L02_today_Y[,c('monthNum')]),]$monthNum)
...
{ print(qm)
  ...
  # Creating data subset
  #
  CPC_L02_ThisMonth       <-subset(CPC_L02_today, as.integer(monthNum)==qm)
  temp_L02_ThisMonth      <-subset(temp_L02, as.integer(monthNum)==qm)
  temp_L00_ThisMonth      <-subset(temp_L00, as.integer(monthNum)==qm)
  ...
  mydata                  <- data.frame(mensile$date,mensile$conc)
  colnames(mydata)        <- c("date","conc")
  ...
  myOutput                <-timeVariation(mydata, pollutant = "conc", ylab = paste("Ntot (#/cm3) -",questo_anno), type="season",
  ...
  TV_ANNUAL_H             <-timeVariation(mydata, pollutant = "conc", ylab = paste("Ntot (#/cm3) -",questo_anno),
  ...
  plot(0:297, ty="n",xaxt = "n",yaxt = "n",
       xlab = paste("Ntot - ",This_Month_Name," ",questo_anno," - monthly trend analysis",sep=""),ylab = "")
  #
  # -------------------------------------------------------------------------------------------
  # Specifying the position of the image through bottom-left and top-right coords
  #
