...
##_______________________________________________________________________________________________________________________##
## Script filename: CPC_D21_1810.R                                                                                       ##
...
# 
if (nchar(Origin_2_n) == 0 && nchar(Origin_3_n) == 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L1_n_lines <- 81 + 0 ; L2_n_lines <- 82 + 0 ; }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n) == 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L1_n_lines <- 81 + 1 ; L2_n_lines <- 82 + 1 ; }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L1_n_lines <- 81 + 2 ; L2_n_lines <- 82 + 2 ; }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n)  > 0 && nchar(Origin_5_n) == 0)  { L1_n_lines <- 81 + 3 ; L2_n_lines <- 82 + 3 ; }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n)  > 0 && nchar(Origin_5_n)  > 0)  { L1_n_lines <- 81 + 4 ; L2_n_lines <- 82 + 4 ; }
#
# -------------------------------------------------------------------------------------------
...
dependent_col,
"1 1 1 1 1
9999.999999 9999.99 9999.99 9999999.999 9.999999999999
...
paste("start_time","end_time","p_int","T_int","conc","numflag"),
...
temp_data$concstp       <- stp_convert(temp_data$conc, p_stp, T_stp)
#
# -------------------------------------------------------------------------------------------
...
for (i in 1:length(temp_data$date)) 
  { if (!is.na(temp_data$calc[i]) & 
        (abs((temp_data$calc[i]-temp_data$calc_mean[i])) 
         > 7*temp_data$calc_sd[i])){ temp_data$nf_bad[i] <- 1 }
  }
#
# -------------------------------------------------------------------------------------------
...
temp_data$concstp [is.na(temp_data$concstp)]  <- 9999999.999
...
#
if (length(temp_data$numflag[temp_data$concstp == 9999999.999]) > 0){
  temp_data$numflag[temp_data$concstp == 9999999.999] <- 0.999    
}
#
# -------------------------------------------------------------------------------------------
...
#
myEBAS                  <- temp_data[,c("start_time","end_time","p_int","T_int","concstp","numflag")]
#
sprintf_formats         <- c(rep("%.6f", 2), rep("%.2f", 2), "%.3f", "%.12f")
...
7
1 1 1 1 1 1 1
9999.999999 9999.99 9999.99 9999999.999 9999999.999 9999999.999 9.999999999999
end_time of measurement, days from the file reference point
...
particle_number_concentration, 1/cm3, Statistics=percentile:15.87 
particle_number_concentration, 1/cm3, Statistics=percentile:84.13 
numflag
0",
(L2_n_lines - 21),
"Data definition:                  EBAS_1.1
...
paste("Comment:                          ",mycomment,sep=""),       
"Acknowledgement:                  Request acknowledgment details from data originator",
paste("start_time","end_time","p_int","T_int","conc","conc_16pc","conc_84pc","numflag"),
...
temp_data_sub           <- subset(temp_data, temp_data$nf_validity == "V", 
                                  select=c("p_int","T_int","concstp","numflag","date"))
#
...
# Calculating 1h data averages
#
lev2_cpc_avg            <- timeAverage(temp_data_sub, avg.time = "hour", statistic = "mean",
                                       start.date=questo_capodanno, 
                                       end.date = as.character(final_time))
lev2_cpc_perc16         <- timeAverage(temp_data_sub, avg.time = "hour", statistic = "percentile", percentile = 15.87,
                                       start.date=questo_capodanno, 
                                       end.date = as.character(final_time))
lev2_cpc_perc84         <- timeAverage(temp_data_sub, avg.time = "hour", statistic = "percentile", percentile = 84.13,                                       
                                       start.date=questo_capodanno, 
                                       end.date = as.character(final_time))
lev2_cpc_n              <- timeAverage(temp_data_sub, avg.time = "hour", statistic = "frequency",
                                       start.date = paste(questo_anno,"01-01",sep = "-"), 
                                       end.date = paste(paste(questo_anno,"12-31",sep = "-"), " 23:59:00",sep = ""))
#
# -------------------------------------------------------------------------------------------
# Calculating JD from average date
#
jd                      <- strptime(lev2_cpc_avg$date, "%Y-%m-%d %H:%M")$yday
h                       <- strptime(lev2_cpc_avg$date, "%Y-%m-%d %H:%M")$hour
time                    <- h*60
time.dec                <- time/1440
lev2_cpc_avg$start_time <-jd+time.dec
lev2_cpc_avg$end_time   <-lev2_cpc_avg$start_time+1/24
#
# Formatting the EBAS format Level-2 matrix
#
EBAS_lev2_cpc           <- data.frame(cbind(lev2_cpc_avg$start_time,
                                            lev2_cpc_avg$end_time,
                                            lev2_cpc_avg$p_int,
                                            lev2_cpc_avg$T_int,
                                            lev2_cpc_avg$concstp,
                                            lev2_cpc_perc16$concstp,
                                            lev2_cpc_perc84$concstp))
colnames(EBAS_lev2_cpc) <- c("start_time","end_time","p_int","T_int","conc","conc_pc16","conc_pc84")
#
# -------------------------------------------------------------------------------------------
# Setting NAs data and flagging data
#
EBAS_lev2_cpc$p_int     [is.na(EBAS_lev2_cpc$p_int)]      <- 9999.99
EBAS_lev2_cpc$T_int     [is.na(EBAS_lev2_cpc$T_int)]      <- 9999.99
EBAS_lev2_cpc[,c(5:7)]  [is.na(EBAS_lev2_cpc[,c(5:7)])]   <- 9999999.999

bad_numflags            <- c("000",as.character(tab_nf$numflag[tab_nf$category != "V"])) 
EBAS_lev2_cpc$numflag   <- sapply(EBAS_lev2_cpc$start_time, 
                                  nf_lev2, 
                                  numflags_mm=myEBAS$numflag, 
                                  startime_mm=myEBAS$start_time,
                                  nv_numflags=bad_numflags)

EBAS_lev2_cpc$numflag   [!is.na(lev2_cpc_n$conc) &
                           lev2_cpc_n$conc >= 30 & 
                           lev2_cpc_n$conc < 45] <- sapply(EBAS_lev2_cpc$numflag [!is.na(lev2_cpc_n$conc) &
                                                                                    lev2_cpc_n$conc >= 30 & 
                                                                                    lev2_cpc_n$conc < 45],
                                                           nf_aggreg, nf_new = 0.392)
EBAS_lev2_cpc$numflag   [!is.na(lev2_cpc_n$conc) & 
                           lev2_cpc_n$conc < 30] <- sapply(EBAS_lev2_cpc$numflag [!is.na(lev2_cpc_n$conc) & 
                                                                                    lev2_cpc_n$conc < 30],
                                                           nf_aggreg, nf_new = 0.390) 

EBAS_lev2_cpc$numflag   [EBAS_lev2_cpc$conc == 9999999.999]   <- 0.999 
#
# -------------------------------------------------------------------------------------------
...
#
sprintf_formats_l2 <- c(rep("%.6f", 2), rep("%.2f", 2), rep("%.3f", 3), "%.12f")
#
EBAS_lev2_cpc[] <- mapply(sprintf, sprintf_formats_l2, EBAS_lev2_cpc)
#
write.table(EBAS_lev2_cpc, file=EBAS_L2_FULLFILENAME,row.names=F,col.names = F, append = T, quote = F,sep=" ")
#
# -------------------------------------------------------------------------------------------
##                                          # END PART 2.3 #
###########################################################################################################################
#                                                                                                                         #
## End of CPC_D21_1810.R                                                                                                  # 
#                                                                                                                         # 
