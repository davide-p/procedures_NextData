...
##_______________________________________________________________________________________________________________________##
## Script filename: NEPH_D22_1810.R                                                                                       ##
...
#
inst_type               <- "NEPH"                                            # replace the value with your instrument type
#
##                                        # END PART 0.2 #
###########################################################################################################################
...
#
temp_L00$p_int  [temp_L00$p_int   == 9999.99]   <- NA
temp_L00$T_int  [temp_L00$T_int   == 9999.99]   <- NA
temp_L00$RH_int [temp_L00$RH_int  == 9999.99]   <- NA
temp_L00$flow   [temp_L00$flow    == 9999.99]   <- NA
#
# -------------------------------------------------------------------------------------------
...
# Setting NA to 9999.999 values
#
temp_L01$p_int  [temp_L01$p_int  == 9999.99]    <- NA
temp_L01$T_int  [temp_L01$T_int  == 9999.99]    <- NA
temp_L01$RH_int [temp_L01$RH_int == 9999.99]    <- NA
#
# -------------------------------------------------------------------------------------------
#Converting JD values to date
#
...
#
NEPH_L02_today          <-data.frame(temp_L02$date, temp_L02$sc450, temp_L02$sc550, temp_L02$sc700, temp_L02$numflag)
#
colnames(NEPH_L02_today)<- c("date","sc450","sc550","sc700","numflag")
#
NEPH_L02_today$month    <- format(NEPH_L02_today$date,"%B")
NEPH_L02_today$monthNum <- as.numeric(format(NEPH_L02_today$date,"%m"))
NEPH_L02_today$day      <- as.numeric(format(NEPH_L02_today$date,"%d"))
#
NEPH_FAIL_today         <- subset(NEPH_L02_today, numflag == 0.999)
#
TFAIL                   <- subset(NEPH_FAIL_today, select=c("month","day","monthNum","numflag"))
...
if (nrow(mesi_FAIL) == 0)  { }
#
NEPH_L02_today_Y        <- subset(NEPH_L02_today, numflag != 0.999)

unico                   <- c(NEPH_L02_today_Y[!duplicated(NEPH_L02_today_Y[,c('month')]),]$month)
...
#
unico_M               <- c(NEPH_L02_today_Y[!duplicated(NEPH_L02_today_Y[,c('monthNum')]),]$monthNum)

for (qm in unico_M)
{
  NEPH_L02_ThisMonth  <-subset(NEPH_L02_today_Y, as.integer(monthNum)==qm)
  ...
  #
  FAIL_ThisMonth      <- subset(NEPH_FAIL_today, as.integer(monthNum) == qm)
  ...
  mensile             <-subset(NEPH_L02_ThisMonth, numflag != 0.999)
  ...
  #
  mensile$mean        <-mean(mensile$sc550)
  mensile$sd          <-sd(mensile$sc550)
  men_MIN             <-subset(mensile, sc550 == min(mensile$sc550))
  men_MAX             <-subset(mensile, sc550 == max(mensile$sc550))
  # -------------------------------------------------------------------------------------------
  # Cleaning Destination directory
  # 
  ...
  # -------------------------------------------------------------------------------------------
  # -------------------------------------------------------------------------------------------
  # Plotting scatt coeff
  #
  plot(ylim=c(men_MIN$sc700[1]-15,men_MAX$sc450[1]+15),
       xlim = c(min(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
        mgp = c(8, 4, 0),
       mensile$date, mensile$sc550, type = "l",
       xlab = "",
       ylab =expression(paste("Scat. coeff. (1/Mm) - L02")), 
       col="darkgreen", 
       panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  title(paste("Scat. coeff. - L02 -",questo_anno,This_Month_Name), line = -3)
  lines(NEPH_L02_ThisMonth$date, mensile$sc450, type = "l", lty = 1, col="darkblue",lwd = 2)
  lines(NEPH_L02_ThisMonth$date, mensile$sc700, type = "l", lty = 1, col="darkred" ,lwd = 2)

  points(men_MAX$date[1], men_MAX$sc550[1], col= "magenta",pch=20)
  text(men_MAX$date[1], men_MAX$sc550[1], labels = paste("Max (", format(men_MAX$date[1],"%B %d"), ")", sep=""),col="magenta",pos=3,cex = 1.8)
  segments(men_MAX$date[1], men_MAX$sc550[1], men_MAX$date[1], men_MIN$sc700[1]-12, lty = 2, col="black",lwd = 1)
  
  points(men_MIN$date[1], men_MIN$sc550[1], col= "blue",pch=20)
  text(men_MIN$date[1], men_MIN$sc550[1], labels = paste("Min (", format(men_MIN$date[1],"%B %d"), ")", sep =""),col="blue",pos=1,cex = 1.8)
  segments(men_MIN$date[1], men_MIN$sc550[1], men_MIN$date[1], men_MIN$sc700[1]-12, lty = 2, col="black",lwd = 1)

  legend("topleft", legend=c("sc450","sc550","sc700"), lty=c(1,1,1), lwd = c(2,2,2), col=c("darkblue","darkgreen","darkred"), cex = 2.5)
  ...
  # -------------------------------------------------------------------------------------------
  # -------------------------------------------------------------------------------------------
  # Plotting Lamp V & Lamp C
  #
  plot(ylim=c(mean(temp_L00_ThisMonth$lamp_c, na.rm = TRUE)-3,mean(temp_L00_ThisMonth$lamp_v, na.rm = TRUE)+3),
       xlim = c(min(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
       mgp = c(8, 4, 0),
       temp_L00_ThisMonth$date, temp_L00_ThisMonth$lamp_v, type = "l", lty = 1, col="darkred",lwd = 2,
       xlab = "",ylab =("Lamp_V (V) and Lamp_C (A) - L00"))
  
  lines(paste("Lamp voltage and current - L00 -",questo_anno,This_Month_Name), line = -3)
  
  title(paste("Flow - L00 -",questo_anno,This_Month_Name), line = -3)
  
  legend("topleft", legend=c("Lamp_V","Lamp_C"), lty=c(1,1), lwd = c(2,2), col=c("darkred","darkblue"), cex = 2.5)
  #
  # -------------------------------------------------------------------------------------------
  # -------------------------------------------------------------------------------------------
  # Plotting P
  #
  if(all(is.na(temp_L00_ThisMonth$p_int)))
  {
    plot(ylim = c(690,890),
         xlim = c(min(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                  max(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
         mgp = c(8, 4, 0),
         temp_L00_ThisMonth$date, temp_L00_ThisMonth$p_int, type = "h",
    ...
    plot(ylim=c(mean(temp_L00_ThisMonth$p_int, na.rm = TRUE)-100,mean(temp_L00_ThisMonth$p_int, na.rm = TRUE)+100),
         xlim = c(min(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                  max(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
         mgp = c(8, 4, 0),
         temp_L00_ThisMonth$date, temp_L00_ThisMonth$p_int, type = "h",
  ...
  # Plotting T_int
  #
  if(all(is.na(temp_L00_ThisMonth$T_int)))
  {
    plot(ylim = c(280,320),
         xlim = c(min(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                  max(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
         mgp = c(8, 4, 0),
         temp_L00_ThisMonth$date, temp_L00_ThisMonth$T_int, type = "h",
    ...
    plot(ylim=c(mean(temp_L00_ThisMonth$T_int, na.rm = TRUE)-20,mean(temp_L00_ThisMonth$T_int, na.rm = TRUE)+20),
         xlim = c(min(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                  max(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
         mgp = c(8, 4, 0),
         temp_L00_ThisMonth$date, temp_L00_ThisMonth$T_int, type = "h",
         xlab = "",ylab =("T_int (K) - L00"), col="pink", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
    lines(temp_L00_ThisMonth$date, temp_L00_ThisMonth$T_int, type = "l", lty = 1, col="blue",lwd = 0.5)
  ...
  if(all(is.na(temp_L00_ThisMonth$RH)))
  {
    ...
    plot(ylim=c(0,100),
         xlim = c(min(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                  max(c(NEPH_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
         mgp = c(8, 4, 0),
         temp_L00_ThisMonth$date, temp_L00_ThisMonth$RH_int, type = "h",
         xlab = "",ylab =("RH (%) - L00"), col="seashell", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
    lines(temp_L00_ThisMonth$date, temp_L00_ThisMonth$RH_int, type = "l", lty = 1, col="mediumorchid4",lwd = 0.5)
  ...
  text(x = 0, y = 0.55, paste("\n", "sc550 min: (", round(min(mensile$sc550),1),") - sc550 max: (", round(max(mensile$sc550),1),") - sc550 mean: (", round(mean(mensile$sc550),1),") - sc550 sd: (", round(sd(mensile$sc550),1),")","\n",
                              "sc550 percentile:    5th: (",round(quantile(mensile$sc550, probs = c(0.05)),digits=1),")",
                              " - 25th: (",round(quantile(mensile$sc550, probs = c(0.25)),digits=1),")",
                              " - 50th: (",round(quantile(mensile$sc550, probs = c(0.50)),digits=1),")",
                              " - 75th: (",round(quantile(mensile$sc550, probs = c(0.75)),digits=1),")",
                              " - 95th: (",round(quantile(mensile$sc550, probs = c(0.95)),digits=1),")","\n","\n",
...
# First semester
#
Fhalf           <-subset(NEPH_L02_today_Y,monthNum<7)
...
if (nrow(Fhalf)==0){ } else 
{
  ...
  {
    print(i)
    mensile            <-subset(NEPH_L02_today_Y, month == i)
    mensile$giorno     <-format(mensile$date,"%d")
    mensile$mean       <-mean(mensile$sc550)
    mensile$sd         <-sd(mensile$sc550)    
    men_MIN            <-subset(mensile, sc550 == min(mensile$sc550))
    men_MAX            <-subset(mensile, sc550 == max(mensile$sc550))
    {
      plot(ylim=c(men_MIN$sc700[1]-15,men_MAX$sc450[1]+15),
           mensile$date, mensile$sc550, type = "l",
           xlab = "",ylab =expression(paste("Scat. coeff. (1/Mm) - L02")), col="darkgreen", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
      title(main=paste(i,questo_anno,sep=" "), 
            line = -2.2)
      
      title(paste("Statistics: min: (", round(min(mensile$sc550),1),") - max: (", round(max(mensile$sc550),1),") - mean: (", round(mean(mensile$sc550),1),") - sd: (", round(sd(mensile$sc550),1),")")
            ,col="black",cex.main = 1.2, line = -3.8, font.main = 1)
      
      lines(mensile$date, mensile$sc450, type = "l", lty = 1, col="darkblue",lwd = 2)
      lines(mensile$date, mensile$sc700, type = "l", lty = 1, col="darkred",lwd = 2)

      points(men_MAX$date[1], men_MAX$sc550[1], col= "magenta",pch=20)
      text(men_MAX$date[1], men_MAX$sc450[1], labels = paste("Max (",format(men_MAX$date[1],"%B %d"),")",sep=""),col="magenta",pos=3)
      segments(men_MAX$date[1], men_MAX$sc550[1], men_MAX$date[1], men_MIN$sc550[1]-12, lty = 2, col="black",lwd = 1)

      points(men_MIN$date[1], men_MIN$sc550[1], col= "blue",pch=20)
      text(men_MIN$date[1], men_MIN$sc700[1], labels = paste("Min (",format(men_MIN$date[1],"%B %d"),")",sep=""),col="blue",pos=1)
      segments(men_MIN$date[1], men_MIN$sc550[1], men_MIN$date[1], men_MIN$sc550[1]-12, lty = 2, col="black",lwd = 1)
      
      legend("topleft", legend=c("sc450","sc550","sc700"), lty=c(1,1,1), lwd = c(2,2,2), col=c("darkblue","darkgreen","darkred"), cex = 1)
    }   
  }
  dev.off() 
}
...
# Second semester
#
Shalf<-subset(NEPH_L02_today_Y,monthNum>6)
...
#
NEPH_L02_today_Y$season[NEPH_L02_today_Y$monthNum>=1  & NEPH_L02_today_Y$monthNum<=3]     <- 1
NEPH_L02_today_Y$season[NEPH_L02_today_Y$monthNum>=4  & NEPH_L02_today_Y$monthNum<=6]     <- 2
NEPH_L02_today_Y$season[NEPH_L02_today_Y$monthNum>=7  & NEPH_L02_today_Y$monthNum<=9]     <- 3
NEPH_L02_today_Y$season[NEPH_L02_today_Y$monthNum>=10 & NEPH_L02_today_Y$monthNum<=12]    <- 4
#
# -------------------------------------------------------------------------------------------
...
#
# -------------------------------------------------------------------------------------------
# Creating temporary season tables (Level-0)
#
...
seasons<-c(NEPH_L02_today_Y[!duplicated(NEPH_L02_today_Y[,c('season')]),]$season)
seasons 
# -------------------------------------------------------------------------------------------
# Preparing plotting files
#  
...
for (stg in seasons)
{
  ...
  NEPH_L02_stg      <-subset(NEPH_L02_today_Y, season == stg)
  ...
  stagionale<-subset(NEPH_L02_stg, numflag != 0.999)
  ...
  stagionale$mean      <- mean(stagionale$sc550)
  stagionale$sd        <- sd(stagionale$sc550)
  stg_MIN              <- subset(stagionale, sc550 == min(stagionale$sc550))
  stg_MAX              <- subset(stagionale, sc550 == max(stagionale$sc550))
  ...
  # Plotting SCAT COEFF
  #
  plot(ylim=c(stg_MIN$sc700[1]-30,stg_MAX$sc450[1]+15),
       stagionale$date, stagionale$sc550, type = "l",
       mgp = c(7, 3, 0),
       xlab = "",ylab =expression(paste("Scat. coeff. (1/Mm) - L02")), col="darkgreen", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 2))
  lines(stagionale$date, stagionale$sc450, type = "l", lty = 1, col="darkblue",lwd = 2)
  lines(stagionale$date, stagionale$sc700, type = "l", lty = 1, col="darkred",lwd = 2)
  
  title(main=paste("Scat. Coeff.",questo_anno,"   ",stg_name), line = -4.5)
  
  mtext(paste("Observations from", OBS_stg_start," to ", OBS_stg_end,
              "(",1+(as.integer(tail((NEPH_L02_stg$date),1)-head((NEPH_L02_stg$date)),1)),"days)       -       Statistics: ",
              "  sca550 min: (", round(min(stagionale$sc550),1),") - sca550 max: (", round(max(stagionale$sc550),1),") - sca550 mean: (", round(mean(stagionale$sc550),1),") - sca550 sd: (", round(sd(stagionale$sc550),1),")"),
                     col="black",cex = 1.7, line = -8.0, font.main = 1)
  
  points(stg_MAX$date[1], stg_MAX$sc550[1], col= "magenta",pch=20)
  segments(stg_MAX$date[1], stg_MAX$sc550[1], stg_MAX$date[1], stg_MIN$sc550[1]-12, lty = 2, col="black",lwd = 1)
  text(stg_MAX$date[1], stg_MIN$sc700[1]-10, labels = paste("Max","(",(format(stg_MAX$date[1],"%B %d")),")"),
       col="magenta",
       pos=1,
       cex = 1.8)
  
  points(stg_MIN$date[1], stg_MIN$sc550[1], col= "blue",pch=20)
  segments(stg_MIN$date[1], stg_MIN$sc550[1], stg_MIN$date[1], stg_MIN$sc550[1]-12, lty = 2, col="black",lwd = 1)
  text(stg_MIN$date[1], stg_MIN$sc700[1]-10, labels = paste("Min","(",(format(stg_MIN$date[1],"%B %d")),")"),
       col="blue",
       pos=1,
       cex = 1.8)
  #
...
# Defining statistics
#
NEPH_L02_today_Y$mean    <-mean(NEPH_L02_today_Y$sc550)
Y_MIN                   <-subset(NEPH_L02_today_Y, sc550 == min(NEPH_L02_today_Y$sc550))
Y_MAX                   <-subset(NEPH_L02_today_Y, sc550 == max(NEPH_L02_today_Y$sc550))
...
# Plotting SCAT COEFF
#
plot(ylim=c(Y_MIN$sc700[1]-15,Y_MAX$sc450[1]+15),
     xlim = c(min(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date))),
     NEPH_L02_today_Y$date, NEPH_L02_today_Y$sc550, type = "l",
     mgp = c(8, 4, 0),
     xlab = "",ylab = expression(paste("Scat. coeff. (1/Mm) - L02")), col="darkgreen", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 2))
title(paste("Scat. coeff. - L02 -",questo_anno), line = -3)
lines(NEPH_L02_today_Y$date, NEPH_L02_today_Y$sc450, type = "l", lty = 1, col="darkblue",lwd = 2)
lines(NEPH_L02_today_Y$date, NEPH_L02_today_Y$sc700, type = "l", lty = 1, col="darkred",lwd = 2)

points(Y_MAX$date[1], Y_MAX$sc550[1], col= "magenta",pch=20)
text(Y_MAX$date[1], Y_MAX$sc450[1], labels = paste0("Max (",format(Y_MAX$date[1],"%B %d"),")"),col="magenta",pos=3,cex = 1.8)
segments(Y_MAX$date[1], Y_MAX$sc550[1], Y_MAX$date[1], Y_MIN$sc700[1]-12, lty = 2, col="black",lwd = 1)

points(Y_MIN$date[1], Y_MIN$sc550[1], col= "blue",pch=20)
text(Y_MIN$date[1], Y_MIN$sc700[1], labels = paste0("Min (",format(Y_MIN$date[1],"%B %d"),")"),col="blue",pos=1,cex = 1.8)
segments(Y_MIN$date[1], Y_MIN$sc550[1],  Y_MIN$date[1], Y_MIN$sc700[1]-12, lty = 2, col="black",lwd = 1)
...
# Plotting Lamp V & Lamp C 
#
plot(ylim=c(mean(temp_L00$lamp_c, na.rm = TRUE)-3,mean(temp_L00$lamp_v, na.rm = TRUE)+3),
     xlim = c(min(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date))),
     temp_L00$date, temp_L00$lamp_v, type = "l", lty = 1, col="darkred",lwd = 2,
     mgp = c(8, 4, 0),
     xlab = "",ylab =("Lamp_V (V) and Lamp_C (A) - L00"))
lines(temp_L00$date, temp_L00$lamp_c, type = "l", lty = 1, col="darkblue",lwd = 2)
title(paste("Lamp voltage and current - L00 -",questo_anno), line = -3)
...
# Plotting P
#
if(all(is.na(temp_L00$p_int)))
{
  plot(ylim = c(690,890),
       xlim = c(min(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date))),
       mgp = c(8, 4, 0),
       temp_L00$date, temp_L00$p_int, type = "h",
       xlab = "",ylab =("P_int (hPa) - L00"), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  title(paste("Internal pressure - L00 - data for this year are absent"), line = -3)
  title(paste("No data"), line = -28, cex = 0.8)
  
} else {
  plot(ylim=c(mean(temp_L00$p_int, na.rm = TRUE)-100,mean(temp_L00$p_int, na.rm = TRUE)+100),
       xlim = c(min(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date))),
       mgp = c(8, 4, 0),
       temp_L00$date, temp_L00$p_int, type = "h",
       xlab = "",ylab =("P_int (hPa) - L00"), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 0.5))
  lines(temp_L00$date, temp_L00$p_int, type = "l", lty = 1, col="darkblue",lwd = 0.5)
  title(paste("Internal pressure - L00 -",questo_anno), line = -3.5)
}
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
# Plotting T_int
#
if(all(is.na(temp_L00$T_int)))
{
  plot(ylim = c(280,320),
       xlim = c(min(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date))),
       mgp = c(8, 4, 0),
       temp_L00$date, temp_L00$T_int, type = "h",
       xlab = "",ylab =("T_int (K) - L00"), col="pink", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  title(paste("Internal temperature - L00 - data for this year are absent"), line = -3)
  title(paste("No data"), line = -28, cex = 0.8)
  
} else {
  plot(ylim=c(mean(temp_L00$T_int, na.rm = TRUE)-20,mean(temp_L00$T_int, na.rm = TRUE)+20),
       xlim = c(min(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date))),
       mgp = c(8, 4, 0),
       temp_L00$date, temp_L00$T_int, type = "h",
       xlab = "",ylab =("T_int (K) - L00"), col="pink", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 0.5))
  lines(temp_L00$date, temp_L00$T_int, type = "l", lty = 1, col="blue",lwd = 0.5)
  title(paste("Internal temperature - L00 -",questo_anno), line = -3.5)
}
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
...
if(all(is.na(temp_L00$RH)))
{
  ...
  plot(ylim=c(0,100),
       xlim = c(min(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(NEPH_L02_today_Y$date,temp_L01$date,temp_L00$date))),
       temp_L00$date, temp_L00$RH_int, type = "h",
       mgp = c(8, 4, 0),
       xlab = "",ylab =("RH (%) - L00"), col="seashell", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  lines(temp_L00$date, temp_L00$RH, type = "l", lty = 1, col="mediumorchid4",lwd = 0.5)
  title(paste("Relative humidity - L00 -",questo_anno), line = -3.5)
}
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
...
text(x = -0.005, y = 0.75, paste(" Observations from", OBS_start," to ", OBS_end,
                                 "\n",
                                 "(days of observation: ", 1+(as.integer(tail((NEPH_L02_today_Y$date),1)-head((NEPH_L02_today_Y$date)),1)),")")
     , cex = 2.8, col = "black",pos = 4)
...
if(is.null(mesi_FAIL_COUNT$print)){
  text(x = 0, y = 0.50, paste("\n", "sca550 min: (", round(min(NEPH_L02_today_Y$sc550),1),") - sca550 max: (", round(max(NEPH_L02_today_Y$sc550),1),") - sca550 mean: (", round(mean(NEPH_L02_today_Y$sc550),1),") - sca550 sd: (", round(sd(NEPH_L02_today_Y$sc550),1),")",
                              "\n",
                              "sca550 percentile:    5th: (",round(quantile(NEPH_L02_today_Y$sc550, probs = c(0.05)),digits=1),")",
                              " - 25th: (",round(quantile(NEPH_L02_today_Y$sc550, probs = c(0.25)),digits=1),")",
                              " - 50th: (",round(quantile(NEPH_L02_today_Y$sc550, probs = c(0.50)),digits=1),")",
                              " - 75th: (",round(quantile(NEPH_L02_today_Y$sc550, probs = c(0.75)),digits=1),")",
                              " - 95th: (",round(quantile(NEPH_L02_today_Y$sc550, probs = c(0.95)),digits=1),")","\n","\n",
...
#
PLOT_C_NAME             <-paste(s_GAW_ID, inst_type,"_SC550_", questo_anno,"CALENDAR_GRAPH",gsub("-","",Sys.Date()),sep = "_")
PLOT_C_NAME_FULL        <-paste (PLOT_DIR_Y,paste(PLOT_C_NAME,".png",sep = ""),sep = "/")

png(file=,PLOT_C_NAME_FULL, width=2100,height=2970,res=250)
calendarPlot(NEPH_L02_today_Y, pollutant = "sc550", year = questo_anno, month=c(1:12), auto.text = TRUE,
             key.footer = expression(paste("Scat. coeff. (1/Mm)")), key.position = "right", key = TRUE,
             main = paste("Scat. coeff. in",questo_anno))
dev.off()
##                                          # END PART 3.4 #
###########################################################################################################################
...
#
mydata                    <-data.frame(NEPH_L02_today_Y$date,
                                       NEPH_L02_today_Y$sc450,
                                       NEPH_L02_today_Y$sc550,
                                       NEPH_L02_today_Y$sc700)
colnames(mydata)          <- c("date","sc450","sc550","sc700")
#
# -------------------------------------------------------------------------------------------
...
png(file=,PLOT_TVH_NAME_FULL, width=930,height=900,res=210)

TV_ANNUAL_H               <-timeVariation(mydata, pollutant = c("sc450","sc550","sc700"), normalise = FALSE, 
                                          ylab = paste("Scat. coeff. (1/Mm) -",questo_anno),
                                          xlab = c("Hour","Month","Weekday"),cols = c("darkblue","darkgreen","darkred"), key = NULL)
...
png(file=,PLOT_TVD_NAME_FULL, width=930,height=1050,res=210)

TV_ANNUAL_H               <-timeVariation(mydata, pollutant = c("sc450","sc550","sc700"), normalise = FALSE, 
                                          ylab = paste("Scat. coeff. (1/Mm) -",questo_anno),
                                          xlab = c("Hour","Month","Weekday"),cols = c("darkblue","darkgreen","darkred"), key = TRUE)
...
png(file=,PLOT_TVM_NAME_FULL, width=930,height=900,res=210)
...
plot(0:297, ty="n",xaxt = "n",yaxt = "n",xlab = paste("Scat. coeff. - ",questo_anno," - annual trend analysis",sep=" "),ylab = "")
#
# -------------------------------------------------------------------------------------------
...
# Cleaning Destination directory
#
unico_T                   <-c(NEPH_L02_today_Y[!duplicated(NEPH_L02_today_Y[,c('monthNum')]),]$monthNum)
...
{ print(qm)
  ...
  # Creating data subset
  #
  NEPH_L02_ThisMonth       <-subset(NEPH_L02_today, as.integer(monthNum)==qm)
  temp_L02_ThisMonth      <-subset(temp_L02, as.integer(monthNum)==qm)
  temp_L00_ThisMonth      <-subset(temp_L00, as.integer(monthNum)==qm)
  ...
  mensile$mean550         <- mean(mensile$sc550)
  mensile$sd550           <- sd(mensile$sc550)
  men_MIN                 <- subset(mensile, sc550 == min(mensile$sc550))
  men_MAX                 <- subset(mensile, sc550 == max(mensile$sc550))
  
  mydata                  <- data.frame(mensile$date,mensile$sc450,mensile$sc550,mensile$sc700)
  colnames(mydata)        <- c("date","sc450","sc550","sc700")
  ...
  myOutput<-timeVariation(mydata, pollutant = c("sc450","sc550","sc700"), ylab = paste("Scat. coeff. (1/Mm) -",questo_anno), type="season",
                          xlab = paste("CMN - month variation \n - ",questo_anno," ",This_Month_Name," -"),
  ...
  png(file=,PLOT_TVH_NAME_FULL, width=1860,height=900,res=210)

  TV_ANNUAL_H             <-timeVariation(mydata, pollutant = c("sc450","sc550","sc700"), ylab = paste("Scat. coeff. (1/Mm) -",questo_anno),
                                          xlab = c("Hour","Hour","Month","Weekday"),cols = c("darkblue","darkgreen","darkred"))
  ...
  png(file=,PLOT_TVD_NAME_FULL, width=930,height=900,res=210)

  TV_ANNUAL_H             <-timeVariation(mydata, pollutant = c("sc450","sc550","sc700"), ylab = paste("Scat. coeff. (1/Mm) -",questo_anno),
                                          xlab = c("Hour","Hour","Month","Weekday"),cols = c("darkblue","darkgreen","darkred"), key = TRUE)
  ...
  plot(0:297, ty="n",xaxt = "n",yaxt = "n",
       xlab = paste("Scat. coeff. - ",This_Month_Name," ",questo_anno," - monthly trend analysis",sep=""),ylab = "")
  #
  # -------------------------------------------------------------------------------------------
  # Specifying the position of the image through bottom-left and top-right coords
  #
