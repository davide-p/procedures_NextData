...
##_______________________________________________________________________________________________________________________##
## Script filename: MAAP_P22_1810.R                                                                                       ##
...
CALIB_DIR       = '../naitza/NEXTDATA/PROD/CIMONE/AEROSOL/MAAP/RAW_DATA_UTC/CALIB'           
RAW_DIR         = '../naitza/NEXTDATA/PROD/CIMONE/AEROSOL/MAAP/RAW_DATA_UTC'
METEO_RAW_DIR   = '../naitza/NEXTDATA/PROD/CIMONE/AEROSOL/NEPH/RAW_DATA_UTC'
#
# -------- DATA DESTINATION PATH --------------------------------------------------------------# REPLACE THE FOLLOWING PATHS
L0_DIR          = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/LEVEL_0'                     
L1_DIR          = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/LEVEL_1' 
L2_DIR          = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/LEVEL_2'
L0_ANCIL_DIR    = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/LEVEL_0/ANCILLARY'
#
# -------- GRAPH DESTINATION PATH -------------------------------------------------------------# REPLACE THE FOLLOWING PATHS
REP_DIR         = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/REPORT'
REP_GRAPH_DIR   = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/REPORT/DAILY_GRAPH'
PLOT_DIR_M      = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/REPORT/MONTHLY_GRAPH'
PLOT_DIR_S      = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/REPORT/SEASONAL_GRAPH'
PLOT_DIR_Y      = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/REPORT/ANNUAL_GRAPH'
PLOT_DIR_Y_PDF  = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/REPORT/ANNUAL_GRAPH/PDF'
PLOT_DIR_T      = '../naitza/NEXTDATA/DISTRIBUTION_DIR/CIMONE/AEROSOL/MAAP/REPORT/TIMEVARIATION_GRAPH' 
#
...
#
inst_type               <- "MAAP"                                             # replace the value with your instrument type
#
##                                        # END PART 0.2 #
###########################################################################################################################
...
# -------------------------------------------------------------------------------------------
# Setting NA to 9999.999 (i.e., invalid) values
#
...
#
MAAP_L02_today          <-data.frame(temp_L02$date,temp_L02$abs670,temp_L02$numflag)
#
colnames(MAAP_L02_today)<- c("date","abs670","numflag")
#
MAAP_L02_today$month    <- format(MAAP_L02_today$date,"%B")
MAAP_L02_today$monthNum <- as.numeric(format(MAAP_L02_today$date,"%m"))
MAAP_L02_today$day      <- as.numeric(format(MAAP_L02_today$date,"%d"))
#
MAAP_FAIL_today         <- subset(MAAP_L02_today, numflag == 0.999)
#
TFAIL                   <- subset(MAAP_FAIL_today, select=c("month","day","monthNum","numflag"))
...
if (nrow(mesi_FAIL) == 0)  { }
#
MAAP_L02_today_Y        <- subset(MAAP_L02_today, numflag != 0.999)
MAAP_L02_today_Y$bc     <- MAAP_L02_today_Y$abs670/6.6

unico                   <- c(MAAP_L02_today_Y[!duplicated(MAAP_L02_today_Y[,c('month')]),]$month)
...
#
unico_M               <- c(MAAP_L02_today_Y[!duplicated(MAAP_L02_today_Y[,c('monthNum')]),]$monthNum)

for (qm in unico_M)
{
  MAAP_L02_ThisMonth  <-subset(MAAP_L02_today_Y, as.integer(monthNum)==qm)
  ...
  #
  FAIL_ThisMonth      <- subset(MAAP_FAIL_today, as.integer(monthNum) == qm)
  ...
  mensile             <-subset(MAAP_L02_ThisMonth, numflag != 0.999)
  ...
  #
  mensile$mean        <-mean(mensile$bc)
  mensile$sd          <-sd(mensile$bc)
  men_MIN             <-subset(mensile, bc == min(mensile$bc))
  men_MAX             <-subset(mensile, bc == max(mensile$bc))
  # -------------------------------------------------------------------------------------------
  # Cleaning Destination directory
  # 
  ...
  # Plotting BC
  #
  plot(ylim=c(men_MIN$bc[1]-0.1,men_MAX$bc[1]+0.1),
       xlim = c(min(c(MAAP_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(MAAP_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
        mgp = c(8, 4, 0),
       mensile$date, mensile$bc, type = "h",
       xlab = "",ylab =expression(paste("BC (",mu,"g/m"^{3},") - L02")), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  title(paste("BC - L02 -",questo_anno,This_Month_Name), line = -3)
  lines(MAAP_L02_ThisMonth$date, mensile$bc, type = "l", lty = 1, col="darkred",lwd = 2)
  
  points(men_MAX$date[1], men_MAX$bc[1], col= "magenta",pch=20)
  text(men_MAX$date[1], men_MAX$bc[1], labels = paste("Max (", format(men_MAX$date[1],"%B %d"), ")", sep=""),col="magenta",pos=3,cex = 1.8)
  segments(men_MAX$date[1], men_MAX$bc[1], men_MAX$date[1], men_MIN$bc[1]-400, lty = 2, col="black",lwd = 1)

  points(men_MIN$date[1], men_MIN$bc[1], col= "blue",pch=20)
  text(men_MIN$date[1], men_MIN$bc[1], labels = paste("Min (", format(men_MIN$date[1],"%B %d"), ")", sep =""),col="blue",pos=1,cex = 1.8)
  segments(men_MIN$date[1], men_MIN$bc[1], men_MIN$date[1], men_MIN$bc[1]-400, lty = 2, col="black",lwd = 1)
  ...
  # Plotting Flow
  #
  plot(ylim=c(mean(temp_L00_ThisMonth$FlowR, na.rm = TRUE)-10,mean(temp_L00_ThisMonth$FlowR, na.rm = TRUE)+10),
       xlim = c(min(c(MAAP_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date)),
                max(c(MAAP_L02_ThisMonth$date,temp_L01_ThisMonth$date,temp_L00_ThisMonth$date))),
       temp_L00_ThisMonth$date, temp_L00_ThisMonth$FlowR, type = "h",
       xlab = "",ylab =("Flow (l/min) - L00"), col="thistle1", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
  lines(temp_L00_ThisMonth$date, temp_L00_ThisMonth$FlowR, type = "l", lty = 1, col="darkred",lwd = 0.5)
  ...
  text(x = 0, y = 0.55, paste("\n", "BC min: (", round(min(mensile$bc),2),") - BC max: (", round(max(mensile$bc),2),") - BC mean: (", round(mean(mensile$bc),2),") - BC sd: (", round(sd(mensile$bc),2),")","\n",
                              "BC percentile:    5th: (",round(quantile(mensile$bc, probs = c(0.05)),digits=2),")",
                              " - 25th: (",round(quantile(mensile$bc, probs = c(0.25)),digits=2),")",
                              " - 50th: (",round(quantile(mensile$bc, probs = c(0.50)),digits=2),")",
                              " - 75th: (",round(quantile(mensile$bc, probs = c(0.75)),digits=2),")",
                              " - 95th: (",round(quantile(mensile$bc, probs = c(0.95)),digits=2),")","\n","\n",
...
# First semester
#
Fhalf           <-subset(MAAP_L02_today_Y,monthNum<7)
...
if (nrow(Fhalf)==0){ } else 
{
  ...
  {
    print(i)
    mensile            <-subset(MAAP_L02_today_Y, month == i)
    mensile$giorno     <-format(mensile$date,"%d")
    mensile$mean       <-mean(mensile$bc)
    mensile$sd         <-sd(mensile$bc)    
    men_MIN            <-subset(mensile, bc == min(mensile$bc))
    men_MAX            <-subset(mensile, bc == max(mensile$bc))
    {
      ...
      title(paste("Statistics: min: (", round(min(mensile$bc),2),") - max: (", round(max(mensile$bc),2),") - mean: (", round(mean(mensile$bc),2),") - sd: (", round(sd(mensile$bc),2),")")
            ,col="black",cex.main = 1.2, line = -3.8, font.main = 1)
      
      lines(mensile$date, mensile$bc, type = "l", lty = 1, col="darkred",lwd = 2)
      
      points(men_MAX$date[1], men_MAX$bc[1], col= "magenta",pch=20)
      text(men_MAX$date[1], men_MAX$bc[1], labels = paste("Max (",format(men_MAX$date[1],"%B %d"),")",sep=""),col="magenta",pos=3)
      segments(men_MAX$date[1], men_MAX$bc[1], men_MAX$date[1], men_MIN$bc[1]-400, lty = 2, col="black",lwd = 1)

      points(men_MIN$date[1], men_MIN$bc[1], col= "blue",pch=20)
      text(men_MIN$date[1], men_MIN$bc[1], labels = paste("Min (",format(men_MIN$date[1],"%B %d"),")",sep=""),col="blue",pos=1)
      segments(men_MIN$date[1], men_MIN$bc[1], men_MIN$date[1], men_MIN$bc[1]-400, lty = 2, col="black",lwd = 1)
    }   
  }
  dev.off() 
...
# Second semester
#
Shalf<-subset(MAAP_L02_today_Y,monthNum>6)
...
#
# -------------------------------------------------------------------------------------------
# Defining periods (JFM, AMJ, JAS, OND)
#
MAAP_L02_today_Y$season[MAAP_L02_today_Y$monthNum>=1  & MAAP_L02_today_Y$monthNum<=3]     <- 1
MAAP_L02_today_Y$season[MAAP_L02_today_Y$monthNum>=4  & MAAP_L02_today_Y$monthNum<=6]     <- 2
MAAP_L02_today_Y$season[MAAP_L02_today_Y$monthNum>=7  & MAAP_L02_today_Y$monthNum<=9]     <- 3
MAAP_L02_today_Y$season[MAAP_L02_today_Y$monthNum>=10 & MAAP_L02_today_Y$monthNum<=12]    <- 4
#
# -------------------------------------------------------------------------------------------
# Creating temporary period tables (Level-2)
#
...
seasons<-c(MAAP_L02_today_Y[!duplicated(MAAP_L02_today_Y[,c('season')]),]$season)
seasons 
# -------------------------------------------------------------------------------------------
# Preparing plotting files
#  
...
for (stg in seasons)
{
  ...
  MAAP_L02_stg      <-subset(MAAP_L02_today_Y, season == stg)
  ...
  stagionale<-subset(MAAP_L02_stg, numflag != 0.999)
  ...
  stagionale$mean      <- mean(stagionale$bc)
  stagionale$sd        <- sd(stagionale$bc)
  stg_MIN              <- subset(stagionale, bc == min(stagionale$bc))
  stg_MAX              <- subset(stagionale, bc == max(stagionale$bc))
  ...
  # Plotting BC
  #
  plot(ylim=c(stg_MIN$bc[1]-0.1,stg_MAX$bc[1]+0.2),
       stagionale$date, stagionale$bc, type = "h",
       mgp = c(7, 3, 0),
       xlab = "",ylab =expression(paste("BC (",mu,"g/m"^{3},") - L02")), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 0.5))
  lines(stagionale$date, stagionale$bc, type = "l", lty = 1, col="darkred",lwd = 1)
  title(main=paste("BC",questo_anno,"   ",stg_name), line = -4.5)
  
  mtext(paste("Observations from", OBS_stg_start,
                           " to ", OBS_stg_end,"(",1+(as.integer(tail((MAAP_L02_stg$date),1)-head((MAAP_L02_stg$date)),1)),"days)       -       Statistics: ",
                           "  BC min: (", round(min(stagionale$bc),2),") - BC max: (", round(max(stagionale$bc),2),") - BC mean: (", round(mean(stagionale$bc),2),") - BC sd: (", round(sd(stagionale$bc),2),")"),
                     col="black",cex = 1.7, line = -8.0, font.main = 1)
  
  points(stg_MAX$date[1], stg_MAX$bc[1], col= "magenta",pch=20)
  segments(stg_MAX$date[1], stg_MAX$bc[1], stg_MAX$date[1], stg_MIN$bc[1]-0.18, lty = 2, col="black",lwd = 1)
  text(stg_MAX$date[1], stg_MIN$bc[1]-0.07, labels = paste("Max","(",(format(stg_MAX$date[1],"%B %d")),")"),
       col="magenta",
       pos=1,
       cex = 1.6)
  
  points(stg_MIN$date[1], stg_MIN$bc[1], col= "blue",pch=20)
  segments(stg_MIN$date[1], stg_MIN$bc[1], stg_MIN$date[1], stg_MIN$bc[1]-0.18, lty = 2, col="black",lwd = 1)
  text(stg_MIN$date[1], stg_MIN$bc[1]-0.07, labels = paste("Min","(",(format(stg_MIN$date[1],"%B %d")),")"),
       col="blue",
       pos=1,
       cex = 1.7)
  #
...
# Defining statistics
#
MAAP_L02_today_Y$mean    <-mean(MAAP_L02_today_Y$bc)
Y_MIN                   <-subset(MAAP_L02_today_Y, bc == min(MAAP_L02_today_Y$bc))
Y_MAX                   <-subset(MAAP_L02_today_Y, bc == max(MAAP_L02_today_Y$bc))
...
# Plotting BC
#
plot(ylim=c(Y_MIN$bc[1]-0.1,Y_MAX$bc[1]+0.2),
     xlim = c(min(c(MAAP_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(MAAP_L02_today_Y$date,temp_L01$date,temp_L00$date))),
     mgp = c(8, 4, 0),
     MAAP_L02_today_Y$date, MAAP_L02_today_Y$bc, type = "h",
     xlab = "",ylab =expression(paste("BC (",mu,"g/m"^{3},") - L02")), col="lightblue", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
title(paste("BC - L02 -",questo_anno), line = -3.5)
lines(MAAP_L02_today_Y$date, MAAP_L02_today_Y$bc, type = "l", lty = 1, col="darkred",lwd = 0.5)

points(Y_MAX$date[1], Y_MAX$bc[1], col= "magenta",pch=20)
text(Y_MAX$date[1], Y_MAX$bc[1], labels = paste0("Max (",format(Y_MAX$date[1],"%B %d"),")"),col="magenta",pos=3,cex = 1.8)
segments(Y_MAX$date[1], Y_MAX$bc[1], Y_MAX$date[1], Y_MIN$bc[1]-0.18, lty = 2, col="black",lwd = 1)

points(Y_MIN$date[1], Y_MIN$bc[1], col= "blue",pch=20)
text(Y_MIN$date[1], Y_MIN$bc[1], labels = paste0("Min (",format(Y_MIN$date[1],"%B %d"),")"),col="blue",pos=1,cex = 1.8)
segments(Y_MIN$date[1], Y_MIN$bc[1], Y_MIN$date[1], Y_MIN$bc[1]-0.18, lty = 2, col="black",lwd = 1)

lines(MAAP_L02_today_Y$date,MAAP_L02_today_Y$mean, type = "l", lty = 1, col="red",lwd = 1)
text(MAAP_L02_today_Y$date[1],MAAP_L02_today_Y$mean[1]+0.05, labels = c("Mean"),pos=2,col="red",cex = 1.5)
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
# Plotting NUMFLAG L01
...
# Plotting Flow
#
plot(ylim=c(mean(temp_L00$FlowR, na.rm = TRUE)-10,mean(temp_L00$FlowR, na.rm = TRUE)+10),
     xlim = c(min(c(MAAP_L02_today_Y$date,temp_L01$date,temp_L00$date)),max(c(MAAP_L02_today_Y$date,temp_L01$date,temp_L00$date))),
     temp_L00$date, temp_L00$FlowR, type = "h",
     xlab = "",ylab =("Flow (l/min) - L00"), col="thistle1", panel.first = grid(nx=0,ny=NULL, lty = 1, lwd = 1))
lines(temp_L00$date, temp_L00$FlowR, type = "l", lty = 1, col="darkred",lwd = 0.5)
title(paste("Flow - L00 -",questo_anno), line = -3)
#
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
# Plotting P
...
text(x = -0.005, y = 0.75, paste(" Observations from", OBS_start," to ", OBS_end,
                                 "\n",
                                 "(days of observation: ", 1+(as.integer(tail((MAAP_L02_today_Y$date),1)-head((MAAP_L02_today_Y$date)),1)),")")
     , cex = 2.8, col = "black",pos = 4)
...
if(is.null(mesi_FAIL_COUNT$print)){
  text(x = 0, y = 0.50, paste("\n", "BC min: (", round(min(MAAP_L02_today_Y$bc),2),") - BC max: (", round(max(MAAP_L02_today_Y$bc),2),") - BC mean: (", round(mean(MAAP_L02_today_Y$bc),2),") - BC sd: (", round(sd(MAAP_L02_today_Y$bc),2),")",
                              "\n",
                              "BC percentile:    5th: (",round(quantile(MAAP_L02_today_Y$bc, probs = c(0.05)),digits=2),")",
                              " - 25th: (",round(quantile(MAAP_L02_today_Y$bc, probs = c(0.25)),digits=2),")",
                              " - 50th: (",round(quantile(MAAP_L02_today_Y$bc, probs = c(0.50)),digits=2),")",
                              " - 75th: (",round(quantile(MAAP_L02_today_Y$bc, probs = c(0.75)),digits=2),")",
                              " - 95th: (",round(quantile(MAAP_L02_today_Y$bc, probs = c(0.95)),digits=2),")","\n","\n",
...
#
calendar_table <- MAAP_L02_today_Y

perc <- quantile(calendar_table$bc, c(0.05, 0.95))

calendar_table$bc[calendar_table$bc < min(perc)] <- min(perc)
calendar_table$bc[calendar_table$bc > max(perc)] <- max(perc)
...
calendarPlot(calendar_table, pollutant = "bc", year = questo_anno, month=c(1:12), auto.text = TRUE,
             key.footer = expression(paste("BC (",mu,"g/m"^{3},")")), key.position = "right", key = TRUE,
             main = paste("BC in",questo_anno))
dev.off()
##                                          # END PART 3.4 #
###########################################################################################################################
...
#
mydata                    <-data.frame(MAAP_L02_today_Y$date,MAAP_L02_today_Y$bc)
colnames(mydata)          <- c("date","bc")
#
# -------------------------------------------------------------------------------------------
...
TV_ANNUAL_H               <-timeVariation(mydata, pollutant = "bc", normalise = FALSE, 
                                          ylab = paste(paste("BC (ug/m3) -",questo_anno),
...
plot(0:297, ty="n",xaxt = "n",yaxt = "n",xlab = paste("BC -",questo_anno,"- annual trend analysis",sep=" "),ylab = "")
#
# -------------------------------------------------------------------------------------------
...
# Cleaning Destination directory
#
unico_T                   <-c(MAAP_L02_today_Y[!duplicated(MAAP_L02_today_Y[,c('monthNum')]),]$monthNum)
...
{ print(qm)
  ...
  # Creating data subset
  #
  MAAP_L02_ThisMonth       <-subset(MAAP_L02_today, as.integer(monthNum)==qm)
  temp_L02_ThisMonth      <-subset(temp_L02, as.integer(monthNum)==qm)
  temp_L00_ThisMonth      <-subset(temp_L00, as.integer(monthNum)==qm)
  ...
  mydata                  <- data.frame(mensile$date,mensile$bc)
  colnames(mydata)        <- c("date","bc")
  ...
  myOutput                <-timeVariation(mydata, pollutant = "bc", ylab = paste(paste("BC (ug/m3) -",questo_anno), type="season",
  ...
  TV_ANNUAL_H             <-timeVariation(mydata, pollutant = "bc", ylab = paste(paste("BC (ug/m3) -",questo_anno),
  ...
  plot(0:297, ty="n",xaxt = "n",yaxt = "n",
       xlab = paste("BC - ",This_Month_Name," ",questo_anno," - monthly trend analysis",sep=""),ylab = "")
  #
  # -------------------------------------------------------------------------------------------
  # Specifying the position of the image through bottom-left and top-right coords
  #
