...
##_______________________________________________________________________________________________________________________##
## Script filename: MAAP_D20_1810.R                                                                                      ##
## Version Date: July 2019
## Jan. 17th, 2019: fixed correct numflag attribution for filter change episodes
...
DAILY_PREFIX    <-"CMN_MAAP"     # choose a prefix for your daily graph (e.g. StationCodeName_ParameterCodeName)
...
## ______________________________________________________________________________________________________________________##
## USE: set the following values to properly link on-site measured Pressure, Temperature and RH values to MAAP dataset
...
#
METEO_DEC_DATE  <- 6          # if different, replace with the position of start_time (julian date) field in your Meteo Raw Data set
METEO_P         <- 16         # if different, replace with the position of Pressure field in your Meteo Raw Data set
METEO_T         <- 17         # if different, replace with the position of Temperature field in your Meteo Raw Data set
METEO_RH        <- 19         # if different, replace with the position of Relative Humidity field in your Meteo Raw Data set
#
# -------------------------------------------------------------------------------------------
...
##                                         Setting MAAP Data
## ______________________________________________________________________________________________________________________##
## USE: set the following values of MAAP dataset
## WARNING: MAAP raw data should be recorded according to the following specifications:
##          FILENAME: the filename should contain the acquisition date (YYYYMMDD). E.g.: CMN_METEO_20181215_01M.dat;
#
# -------- MAAP RAW DATASET EXTENSION ----------------------------------------------------------# REPLACE THE FOLLOWING VALUES
#
MAAP_EXT         <-".dat"      # if different, replace ".dat" with the extesion of your MAAP Raw Data set
#
# -------- MAAP RAW DATA FIELD SEPARATOR -------------------------------------------------------# REPLACE THE FOLLOWING VALUES
#
MAAP_FIELD_SEP   <-" "         # if different, replace " "    with the field separator of your MAAP Raw Data set (e.g. "," or "\t")
#
# -------- MAAP RAW DATA HEADER ----------------------------------------------------------------# REPLACE THE FOLLOWING VALUES
#
MAAP_FIELD_NAM   <- T          # if different, replace T      with F if MAAP Row Data tables do not have the header (field names)
#
# -------- MAAP FIELD POSITION IN THE TABLE -----------------------------------------------------# REPLACE THE FOLLOWING VALUES
#
MAAP_DEC_DATE       <- 6          # if different, replace with the position of start_time (julian date) field in your MAAP Raw Data set
MAAP_SFlow          <- 15         # if different, replace with the position of Sflow field in your MAAP Raw Data set
MAAP_BC_mass        <- 24         # if different, replace with the position of BC_mass field in your MAAP Raw Data set
MAAP_BC_mass_filter <- 19         # if different, replace with the position of BC_mass_filter field in your MAAP Raw Data set
MAAP_Status_c1      <- 30         # if different, replace with the position of Status_c1 field in your MAAP Raw Data set
#
# -------------------------------------------------------------------------------------------
...
inst_type               <- "filter_absorption_photometer"                    # replace the value with your instrument type
inst_manu               <- "Thermo"                                          # replace the value with your instrument manufacter
inst_modl               <- "5012"                                            # replace the value with your instrument model
inst_name               <- "MAAP01"                                          # replace the value with your your instrument name
inst_s_n                <- "080"                                             # replace the value with your instrument serial number
meth_ref                <- "IT06L_MAAP01_met"                                # replace the value with method reference

dependent_col           <- "9"                                               # replace the value with the total number of columns of the file in addition to start_time (i.e., total-1)

component               <- "black_carbon"                                    # replace the value with proper component 
matrix                  <- "aerosol"                                         # replace the value with proper matrix
meas_unit               <- "ug/m3"                                           # replace the value with proper measurement unit
...
detec_lim               <- "0.1 ug/m3"                                       # replace the value with Detection limit
...
meas_uncr               <- " "                                               # replace the value with Measurement uncertainty
...
std_meth                <- "Multi-angle_Correction=Petzold2004"              # replace the value with Standard method
qa_mes_id               <- "WCCAP-AP-2016-1-6"                               # replace the value with QA measure ID
...
#
if (nchar(Origin_2_n) == 0 && nchar(Origin_3_n) == 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L0_n_lines <- 84 + 0 }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n) == 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L0_n_lines <- 84 + 1 }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L0_n_lines <- 84 + 2 }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n)  > 0 && nchar(Origin_5_n) == 0)  { L0_n_lines <- 84 + 3 }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n)  > 0 && nchar(Origin_5_n)  > 0)  { L0_n_lines <- 84 + 4 }
#
# -------------------------------------------------------------------------------------------
...
# 
param_code              <- "aerosol.black_carbon.pm10.1y.1mn"
level_code              <- "lev0.nas"                                       
...
#questo_anno             <-as.numeric(questo_anno) -1   # the value "-1" means "last year". Change to "-2" or "-3" etc. for previous years
...
# NOTE: the following process creates new EBAS Level-0 files within the destination directory
#
...
dependent_col,
"1 1 1 1 1 1 1 1 1 
999.999999  9999.99 9999.99 9999.99 999.99 0x999999 9999.99999999 9999.99999999 9.999999999
...
equivalent_black_carbon, ug/m3, Wavelength=670 nm, Measurement uncertainty=6%
equivalent_black_carbon_loading, ug, Wavelength=670 nm, Measurement uncertainty=6%
numflag
0",
(L0_n_lines - 23),
...
paste("start_time","end_time","p_int","T_int","RH","FlowR","SFlag","BCconc670","BCmass670","numflag"),
...
#
# -------------------------------------------------------------------------------------------
# Listing the MAAP Raw data
#
lsfiles                 <-file.info(dir(RAW_DIR, pattern = glob2rx(paste0("*",MAAP_EXT)), full.names = F, ignore.case = TRUE))
...
#
# -------------------------------------------------------------------------------------------
# Creating temporary MAAP dataset for current year
# 
for(j in df_lista$fileName) {
  
  ULTIMO_DATO           <-paste(RAW_DIR,j, sep="/")
  ULTIMO_DATO_NAME      <-basename(ULTIMO_DATO)
  TABELLA               <-read.table(file=ULTIMO_DATO,fill = T, header = MAAP_FIELD_NAM, row.names=NULL,
                                     colClasses = c(rep("numeric",6),rep("character",25)))
  
  names(TABELLA)[MAAP_DEC_DATE]       <-"start_time"
  names(TABELLA)[MAAP_SFlow]          <-"SFlow"
  names(TABELLA)[MAAP_BC_mass]        <-"BC_mass"
  names(TABELLA)[MAAP_BC_mass_filter] <-"BC_mass_filter"
  names(TABELLA)[MAAP_Status_c1]      <-"Status_c1"

  TABELLA               <-TABELLA[,c("start_time","SFlow", "BC_mass", "BC_mass_filter", "Status_c1")]
  ...
  TABELLA               <-TABELLA[,c("code","start_time","end_time","SFlow", "BC_mass", "BC_mass_filter", "Status_c1")]
...
colnames(TABELLA)       <-c("code","start_time","end_time","SFlow", "BC_mass", "BC_mass_filter", "Status_c1")
#
# -------------------------------------------------------------------------------------------
# Merging MAAP and METEO Data
#
myEBAS                  <- merge(x = TABELLA[,c("code","start_time","end_time","SFlow", "BC_mass", "BC_mass_filter", "Status_c1")],
                                 y = meteo_data[,c("code","P","T","RH")], by = "code", all.x = TRUE)
myEBAS                  <- myEBAS[order(myEBAS$start_time),]
myEBAS                  <- myEBAS[,c("start_time","end_time","P","T","RH","SFlow", "BC_mass", "BC_mass_filter", "Status_c1")]
#
# -------------------------------------------------------------------------------------------
# Setting MAAP and METEO variables
#
myEBAS$SFlow            <- as.numeric(unlist(myEBAS$SFlow))
myEBAS$BC_mass          <- as.numeric(unlist(myEBAS$BC_mass))
myEBAS$BC_mass_filter   <- as.numeric(unlist(myEBAS$BC_mass_filter))
myEBAS$T                <- as.numeric(unlist(myEBAS$T))
myEBAS$SFlow            <- myEBAS$SFlow * 1000
myEBAS$T                <- myEBAS$T     + 273.15
#
# -------------------------------------------------------------------------------------------
# Adding new fields
#
myEBAS$Status           <- paste0("0x",as.character(myEBAS$Status_c1))
myEBAS$numflag          <- 0
#
# -------------------------------------------------------------------------------------------
...
myEBAS$SFlow[is.na(myEBAS$SFlow)]                   <- 99.99
myEBAS$BC_mass[is.na(myEBAS$BC_mass)]               <- 9999.99999999  
myEBAS$BC_mass_filter[is.na(myEBAS$BC_mass_filter)] <- 9999.99999999 
#
# -------------------------------------------------------------------------------------------
# Flagging MAAP values:
# See PART 0.1 for the setting of the numflag functions and table
myEBAS$numflag[myEBAS$RH > 40 & myEBAS$RH != 9999.99] <- sapply(myEBAS$numflag [myEBAS$RH > 40 & myEBAS$RH != 9999.99], 
                                                                nf_aggreg, nf_new = 0.640) 

myEBAS$numflag[myEBAS$BC_mass == 9999.99999999] <- sapply(myEBAS$numflag[myEBAS$BC_mass == 9999.99999999],
                                                                                                   nf_aggreg, nf_new = 0.980)
#
# -------------------------------------------------------------------------------------------
# Removing possibile duplicates
#
myEBAS                  <- myEBAS[,c("start_time",
                                     "end_time",
                                     "P",
                                     "T",
                                     "RH",
...
sprintf_formats                                       <- c(rep("%.6f", 2), rep("%.2f", 4), "%s", rep("%.8f", 2), "%.9f")
myEBAS                                                <- myEBAS
...
#
REPORTAB            <-myEBAS[, c("start_time","BC_mass","BC_mass_filter","P","T","RH","SFlow")]
...
for (d in reportday)
{
  ...
  {png(file = ULTIMO_DATO_PNG,width=10000,height=15000,res=1000)
    timePlot(THISREPORTTAB,pollutant=c("BC_mass",
                                       "BC_mass_filter",
                                       "P",
                                       "T",
                                       "RH",
