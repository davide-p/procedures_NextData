...
##_______________________________________________________________________________________________________________________##
## Script filename: OPC_P20_1810.R                                                                                       ##
...
DAILY_PREFIX    <-"CMN_OPC"     # choose a prefix for your daily graph (e.g. StationCodeName_ParameterCodeName)
...
## ______________________________________________________________________________________________________________________##
## USE: set the following values to properly link on-site measured Pressure, Temperature and RH values to OPC dataset
...
#
METEO_FIELD_NAM <- T          # if different, replace "T" with "F" if Meteo Row Data tables do not have the header (filed names)
#
...
##                                         Setting OPC Data
## ______________________________________________________________________________________________________________________##
## USE: set the following values of OPC dataset
## WARNING: OPC raw data should be recorded according with the following specifics:
##          FILENAME: the filename should contain the acquisition date (YYYYMMDD). E.g.: CMN_METEO_20181215_01M.dat;
#
# -------- OPC RAW DATASET EXTENSION ----------------------------------------------------------# REPLACE THE FOLLOWING VALUES
#
OPC_EXT         <-".dat"      # if different, replace ".dat" with the extesion of your OPC Raw Data set
#
# -------- OPC RAW DATA FIELD SEPARATOR -------------------------------------------------------# REPLACE THE FOLLOWING VALUES
#
OPC_FIELD_SEP   <-" "         # if different, replace " "    with the field separator of your OPC Raw Data set (e.g. "," or "\t")
#
# -------- OPC RAW DATA HEADER ----------------------------------------------------------------# REPLACE THE FOLLOWING VALUES
#
OPC_FIELD_NAM   <- F          # if different, replace F      with F if OPC Raw Data tables do not have the header (filed names)
#
# -------- OPC FIELD POSITION IN THE TABLE -----------------------------------------------------# REPLACE THE FOLLOWING VALUES
#
OPC_DEC_DATE  <- 6          # if different, replace with the field position of start_time (julian date) field in your OPC Raw Data set
OPC_0_30um    <- 7          # if different, replace with the field position of >0.30um measure
OPC_0_40um    <- 8          # if different, replace with the field position of >0.40um measure
OPC_0_50um    <- 9          # if different, replace with the field position of >0.50um measure
OPC_0_65um    <- 10         # if different, replace with the field position of >0.65um measure
OPC_0_80um    <- 11         # if different, replace with the field position of >0.65um measure  
OPC_1_00um    <- 12         # if different, replace with the field position of >1.00um measure
OPC_1_60um    <- 13         # if different, replace with the field position of >1.60um measure
OPC_2_00um    <- 14         # if different, replace with the field position of >2.00um measure
OPC_3_00um    <- 15         # if different, replace with the field position of >3.00um measure
OPC_4_00um    <- 16         # if different, replace with the field position of >4.00um measure
OPC_5_00um    <- 17         # if different, replace with the field position of >5.00um measure
OPC_7_50um    <- 18         # if different, replace with the field position of >7.50um measure
OPC_10_0um    <- 19         # if different, replace with the field position of >10.0um measure
OPC_15_0um    <- 20         # if different, replace with the field position of >20.0um measure
OPC_20_0um    <- 21         # if different, replace with the field position of >20.0um measure
#
# -------------------------------------------------------------------------------------------
...
#
if (nchar(Origin_2_n) == 0 && nchar(Origin_3_n) == 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L0_n_lines <- 125 + 0 }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n) == 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L0_n_lines <- 125 + 1 }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n) == 0 && nchar(Origin_5_n) == 0)  { L0_n_lines <- 125 + 2 }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n)  > 0 && nchar(Origin_5_n) == 0)  { L0_n_lines <- 125 + 3 }
if (nchar(Origin_2_n)  > 0 && nchar(Origin_3_n)  > 0 && nchar(Origin_4_n)  > 0 && nchar(Origin_5_n)  > 0)  { L0_n_lines <- 125 + 4 }
#
# -------------------------------------------------------------------------------------------
...
# 
param_code              <- "optical_particle_size_spectrometer.particle_number_size_distribution.aerosol.1y.1mn"
level_code              <- "lev0.nas"                                       
...
# -------------------------------------------------------------------------------------------
# Listing the OPC Raw data
#

lsfiles                 <-file.info(dir(RAW_DIR, pattern = glob2rx(paste0("*",OPC_EXT)), full.names = F, ignore.case = TRUE))
...
ndata <- NROW(df_lista)
# -------------------------------------------------------------------------------------------
# Creating temporary OPC dataset for current year
# 
...
for(j in df_lista$fileName) {
  ...
  TABELLA               <-read.table(file=ULTIMO_DATO,fill = T, header = OPC_FIELD_NAM, skip = 1, row.names=NULL)
  
  print(sapply(TABELLA, class))
  
  names(TABELLA)[OPC_DEC_DATE]  <-"start_time"
  # -------------------------------------------------------------------------------------------
  # Calculating channel values as differential counts (original values are total counts)

  TABELLA$bin_01            <- (TABELLA)[,OPC_0_30um] - (TABELLA)[,OPC_0_40um]
  TABELLA$bin_02            <- (TABELLA)[,OPC_0_40um] - (TABELLA)[,OPC_0_50um]
  TABELLA$bin_03            <- (TABELLA)[,OPC_0_50um] - (TABELLA)[,OPC_0_65um]
  TABELLA$bin_04            <- (TABELLA)[,OPC_0_65um] - (TABELLA)[,OPC_0_80um]
  TABELLA$bin_05            <- (TABELLA)[,OPC_0_80um] - (TABELLA)[,OPC_1_00um]
  TABELLA$bin_06            <- (TABELLA)[,OPC_1_00um] - (TABELLA)[,OPC_1_60um]
  TABELLA$bin_07            <- (TABELLA)[,OPC_1_60um] - (TABELLA)[,OPC_2_00um]
  TABELLA$bin_08            <- (TABELLA)[,OPC_2_00um] - (TABELLA)[,OPC_3_00um]
  TABELLA$bin_09            <- (TABELLA)[,OPC_3_00um] - (TABELLA)[,OPC_4_00um]
  TABELLA$bin_10            <- (TABELLA)[,OPC_4_00um] - (TABELLA)[,OPC_5_00um]
  TABELLA$bin_11            <- (TABELLA)[,OPC_5_00um] - (TABELLA)[,OPC_7_50um]
  TABELLA$bin_12            <- (TABELLA)[,OPC_7_50um] - (TABELLA)[,OPC_10_0um]
  TABELLA$bin_13            <- (TABELLA)[,OPC_10_0um] - (TABELLA)[,OPC_15_0um]
  TABELLA$bin_14            <- (TABELLA)[,OPC_15_0um] - (TABELLA)[,OPC_20_0um]
  TABELLA$bin_15            <- (TABELLA)[,OPC_20_0um]
  TABELLA$fine              <- (TABELLA)[,OPC_0_30um] - (TABELLA)[,OPC_1_00um]
  TABELLA$coarse            <- (TABELLA)[,OPC_1_00um]
  #
  TABELLA               <-TABELLA[,c("start_time","bin_01","bin_02","bin_03",
                                     "bin_04","bin_05","bin_06","bin_07","bin_08",
                                     "bin_09","bin_10","bin_11","bin_12","bin_13",
                                     "bin_14","bin_15","fine","coarse")]
  ...
  TABELLA               <-TABELLA[,c("start_time","end_time",
                                     "bin_01","bin_02","bin_03",
                                     "bin_04","bin_05","bin_06","bin_07","bin_08",
                                     "bin_09","bin_10","bin_11","bin_12","bin_13",
                                     "bin_14","bin_15","fine","coarse","numflag","code")]
...
colnames(TABELLA)       <-c("start_time","end_time",
                            "bin_01","bin_02","bin_03",
                            "bin_04","bin_05","bin_06","bin_07","bin_08",
                            "bin_09","bin_10","bin_11","bin_12","bin_13",
                            "bin_14","bin_15","fine","coarse","numflag","code")
...
# -------------------------------------------------------------------------------------------
# Merging OPC and METEO Data
#
myEBAS <- merge(x = TABELLA[,c("code","start_time","end_time",
                               "bin_01","bin_02","bin_03",
                               "bin_04","bin_05","bin_06","bin_07","bin_08",
                               "bin_09","bin_10","bin_11","bin_12","bin_13",
                               "bin_14","bin_15","fine","coarse","numflag")], 
                y = meteo_data[,c("code","P","T","RH")], by = "code", all.x = TRUE)

myEBAS <- myEBAS[order(myEBAS$start_time),]

myEBAS <- myEBAS[,c("start_time","end_time","P","RH","T",
                    "bin_01","bin_02","bin_03",
                    "bin_04","bin_05","bin_06","bin_07","bin_08",
                    "bin_09","bin_10","bin_11","bin_12","bin_13",
                    "bin_14","bin_15","fine","coarse","numflag")]
...
myEBAS$T    [is.na(myEBAS$T)]     <- 9999.9
...
myEBAS$numflag[myEBAS$RH > 40 & myEBAS$RH != 999.9] <- sapply(myEBAS$numflag[myEBAS$RH > 40 & myEBAS$RH != 999.9],
...
for (d in reportday)
{
  ...
  if (file.exists(ULTIMO_DATO_PNG)){} 
    ...
    timePlot(THISREPORTTAB,pollutant=c("fine",
                                       "coarse",
                                       "P",
                                       "T",
                                       "RH"
